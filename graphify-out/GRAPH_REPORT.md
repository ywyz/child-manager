...
## Summary
- 2558 nodes · 2825 edges · 750 communities (182 shown, 568 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 67 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `caad926a`
...
1. `IdentityRepository` - 39 edges
2. `IdentityError` - 36 edges
3. `ContractModel` - 32 edges
4. `IdentityService` - 31 edges
5. `create_app()` - 25 edges
6. `AuditRepository` - 25 edges
...
- `Spec Kit Refresh Tokens` --semantically_similar_to--> `Refresh Tokens`  [INFERRED] [semantically similar]
  specs/001-daily-activity-plan/data-model.md → docs/design/data-model.md
- `Spec Kit Transaction Invariants` --semantically_similar_to--> `Application-Owned Transaction Boundary`  [INFERRED] [semantically similar]
  specs/001-daily-activity-plan/data-model.md → docs/design/data-model.md
...
- **Canonical Data Model Alignment** — docs_design_data_model_child_manager_data_model, docs_design_database_schema_child_manager_postgresql_schema, specs_001_daily_activity_plan_data_model_phase_1_data_model [INFERRED 0.95]
- **Refresh Token Family Semantics** — docs_design_data_model_refresh_token_family_absolute_expiry, docs_design_database_schema_refresh_token_family_consistency_transaction, specs_001_daily_activity_plan_data_model_refresh_token_family_absolute_expiry, docs_design_data_model_refresh_token_family_revocation, specs_001_daily_activity_plan_data_model_refresh_token_family_revocation [INFERRED 0.95]

## Communities (750 total, 568 thin omitted)

### Community 0 - "MemoryLoginThrottle"
Cohesion: 0.17
Nodes (10): _login_throttle(), MemoryLoginThrottle, datetime, Redis 有界窗口实现；测试可使用 MemoryLoginThrottle 确定性替身。, RedisLoginThrottle, Redis, test_account_delay_starts_at_fifth_failure_and_is_exponential(), test_redis_throttle_persists_source_limit_across_instances() (+2 more)
...
### Community 14 - "test_auth_smoke.py"
Cohesion: 0.07
...
### Community 16 - "IdentityError"
Cohesion: 0.26
Nodes (9): Exception, AuditRepository, AuthResult, IdentityError, IdentityService, Connection, UUID, _session_version() (+1 more)
...
### Community 41 - "IdentityRepository"
Cohesion: 0.18
...
### Community 45 - "auth.py"
Cohesion: 0.06
Nodes (67): AdminSessionDependency, _allowed_origins(), change_password(), _clear_auth_cookies(), _cookie_secure(), csrf(), login(), logout() (+59 more)

### Community 46 - "service.py"
Cohesion: 0.20
Nodes (13): hash_password(), password_violations(), Path, verify_password(), _weak_passwords(), _native_url(), 身份用例：实时授权、会话轮换、撤销与最后管理员保护。, Path (+5 more)
...
### Community 132 - "test_auth.py"
Cohesion: 0.11
Nodes (35): initialize_admin(), _native_url(), 首位管理员初始化必须在单一数据库事务内完成。, 返回 True 表示创建成功，False 表示系统已经初始化。, _init_admin(), main(), csrf_headers(), identity_client() (+27 more)
...
### Community 314 - "Child Manager Data Model"
Cohesion: 0.20
Nodes (19): Application-Owned Transaction Boundary, Child Manager Data Model, Kindergarten Isolation, PostgreSQL as Production Source, Refresh Token Family Absolute Expiry, Refresh Token Family Revocation, Refresh Tokens, Users (+11 more)

### Community 325 - "create_app"
Cohesion: 0.29
...
### Community 549 - "IdentityAuditEventCode"
Cohesion: 0.21
Nodes (8): Connection, UUID, AuditEventReference, IdentityAuditEventCode, ResourceReference, StrEnum, test_identity_audit_rejects_sensitive_metadata_keys(), test_identity_audit_repository_is_append_only()
...
### Community 720 - "build_health_dependencies"
Cohesion: 0.24
...
### Community 722 - "hash_refresh_token"
Cohesion: 0.32
Nodes (11): create_access_token(), decode_access_token(), generate_refresh_token(), hash_refresh_token(), Any, datetime, Access JWT 与 opaque Refresh token 接缝。, test_refresh_and_logout_do_not_resolve_token_outside_active_kindergarten() (+3 more)
...
### Community 724 - "test_identity_isolation.py"
Cohesion: 0.45
Nodes (11): identity_database(), _insert_kindergarten(), _insert_user(), Connection, MonkeyPatch, UUID, test_cross_kindergarten_role_assignment_is_rejected_by_composite_foreign_key(), test_refresh_replacement_cannot_cross_kindergarten() (+3 more)
...
### Community 727 - "ADR-0002：采用独立 Web、API、Worker 运行单元的模块化单体"
Cohesion: 0.18
...
### Community 746 - "app.py"
Cohesion: 0.31
Nodes (7): _error_response(), Request, UUID, FastAPI 应用装配、统一异常转换与健康端点。, _request_id(), JSONResponse, FieldError

### Community 747 - "dependencies.py"
Cohesion: 0.22
Nodes (6): admin_session(), current_session(), identity_service(), CurrentSessionDependency, IdentityServiceDependency, Cookie

### Community 748 - "normalize_username"
Cohesion: 0.46
...
## Knowledge Gaps
- **1430 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+1425 more)
...
- **Are the 5 inferred relationships involving `IdentityError` (e.g. with `create_app()` and `HealthDependencies`) actually correct?**
  _`IdentityError` has 5 INFERRED edges - model-reasoned connections that need verification._
- **Are the 18 inferred relationships involving `ContractModel` (e.g. with `AuditEventReference` and `IdentityAuditEventCode`) actually correct?**
  _`ContractModel` has 18 INFERRED edges - model-reasoned connections that need verification._
