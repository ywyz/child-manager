...
## Summary
- 494 nodes · 465 edges · 104 communities (50 shown, 54 thin omitted)
...
3. `Child Manager Agent 开发规则` - 19 edges
4. `Child Manager 项目上下文` - 15 edges
5. `Child Manager 幼儿园教育管理系统` - 14 edges
6. `这是一份word表格` - 11 edges
7. `右侧列` - 11 edges
8. `7. 共同实施路线` - 10 edges
9. `18. 验收标准` - 9 edges
10. `填写说明` - 9 edges
...
### Community 0 - "FastAPI API 运行单元"
Cohesion: 0.08
Nodes (33): AI 最多三次调用策略, API 起始的信任边界, 应用用例深模块接口, 稳定跨进程契约包, 敏感数据最小化, 功能完成前延后生产部署, Dramatiq Worker 运行单元, 外部故障降级与权威状态恢复 (+25 more)
...
## Knowledge Gaps
- **349 isolated node(s):** `1. 事实来源与冲突处理`, `2.1 先理解再编码`, `2.2 保持最小实现`, `2.3 先定义完成标准`, `3. 精准修改` (+344 more)
...
- **Why does `教案管理 PRD（首期：一日活动计划）` connect `教案管理 PRD（首期：一日活动计划）` to `README.md`, `5. 用户与权限`, `8. 核心业务对象`, `18. 验收标准`, `15. 安全、隐私与审计`, `4. 产品目标`, `6. 首期范围`, `10. 教案栏目与格式`, `13. AI 生成流程`, `12. AI 模型与提示词管理`, `7. 首期必要设置`, `9. 日期与上下文规则`, `11. 计划管理流程`, `14. Word 导出`, `17. 非功能要求`?**
  _High betweenness centrality (0.211) - this node is a cross-community bridge._
- **Why does `Child Manager 数据模型设计` connect `Child Manager 数据模型设计` to `15. 数据快照边界`, `README.md`, `22. 后续扩展边界`, `2. 设计来源与旧系统取舍`, `8. 提示词模型`, `11. 后台任务与 AI 结果`, `16. 外键与删除行为`, `4. 模型总览`, `7. AI 模型档案`, `3. 总体建模原则`, `9. 一日活动计划模型`, `5. 园所与身份模型`, `6. 教学设置模型`, `21. 测试与验收`?**
...
- **Should `FastAPI API 运行单元` be split into smaller, more focused modules?**
