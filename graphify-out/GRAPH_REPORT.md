...
Cohesion: 0.05
Nodes (84): _ai_unconfigured(), build_health_dependencies(), _calendar_library_available(), check_csrf(), get_current_user(), get_db(), Depends, Request (+76 more)
...
Cohesion: 0.15
Nodes (10): AuditEvent, _now(), datetime, AuditRepository, Any, AuditService, Any, 业务审计门面，隐藏 Repository 细节并约束身份审计字段。 (+2 more)
...
Cohesion: 0.16
Nodes (18): create_access_token(), decode_access_token(), generate_refresh_value(), hash_refresh_value(), parse_refresh_kindergarten_id(), Any, Access JWT 与 opaque Refresh 令牌。, 签发 Access JWT。      family_id 与 Refresh Token family 绑定，用于在退出、改密、重置、停用或     Refr (+10 more)
...
Cohesion: 0.21
Nodes (9): normalize_phone(), normalize_username(), 返回规范化用户名：NFKC、去首尾空白、小写。, 返回 E.164 手机号或空值；当前只接受中国大陆手机号。, Any, 幂等确保园所具备 admin/teacher 角色。, test_normalize_phone_e164_or_empty(), test_normalize_phone_rejects_invalid() (+1 more)
...
Cohesion: 0.15
Nodes (7): RefreshToken, datetime, 在指定园所内通过 token_hash 定位 Refresh Token。          Refresh 明文已编码园所 ID，调用方解析后必须显式传入 k, 带行锁的 find_refresh_token_by_hash；隔离语义同上。, 撤销单个 Refresh Token（用于正常轮换）。, 撤销整个 Refresh family（用于退出、改密、重置、停用、重放）。, 撤销用户的全部 Refresh Token 及其 family。
...
Cohesion: 0.23
Nodes (14): IdentityService, Access Token 携带的 family 是否仍活跃。, 身份业务编排：协调 Repository、Token、CSRF 与审计。, _audit_count(), _commit(), kindergarten(), MonkeyPatch, 审计持久化测试：关键身份事件在失败路径下仍写入 audit_events。 (+6 more)

### Community 197 - "select"
Cohesion: 0.18
Nodes (4): Role, 返回具有 admin 角色的有效用户数量。, family 内存在未撤销的 token 时视为活跃。, select
...
### Community 199 - "User"
Cohesion: 0.22
Nodes (16): Match, User, Pattern, _add_js_rules(), _js_rule(), Any, NiceGUI 页面级冒烟测试。  使用 nicegui.testing.User 驱动真实页面，验证登录、改密、账号管理和导航 页面渲染及关键交互；页面内的, 为页面中 run_javascript 发起的 API 调用注入 mock 响应。 (+8 more)

### Community 200 - "session.py"
Cohesion: 0.20
Nodes (7): kindergarten(), MonkeyPatch, Refresh Token family 语义测试。, 在已迁移的隔离 schema 上构造 IdentityService。, service(), _set_keys(), test_refresh_preserves_family_id_and_absolute_expiration()
...
- **Why does `IdentityService` connect `IdentityService` to `auth.py`, `tokens.py`, `session.py`, `init_admin.py`, `test_auth_smoke.py`, `AuditService`, `._record_identity`, `create_app`, `.create_user`, `validate_password`?**
