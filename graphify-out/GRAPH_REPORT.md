...
## Corpus Check
- 158 files · ~85,045 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1770 nodes · 2110 edges · 183 communities (175 shown, 8 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 37 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `536f149f`
...
5. `create_app()` - 20 edges
6. `Child Manager Agent 开发规则` - 20 edges
7. `Child Manager PostgreSQL 数据库 Schema` - 19 edges
8. `Child Manager 产品与工程路线图` - 18 edges
9. `HealthDependencies` - 17 edges
10. `Tasks: 首期一日活动计划完整闭环` - 17 edges
...
- `test_health_response_validation()` --calls--> `HealthResponse`  [EXTRACTED]
  tests/contract/test_common_contracts.py → packages/contracts/common.py
...
Cohesion: 0.08
Nodes (24): ErrorResponse, FieldError, PaginatedResponse, BaseModel, RequestContext, ErrorResponse 必须拒绝非 UUID 的 request_id。, ErrorResponse 不接受额外字段。, PaginatedResponse 不接受额外字段。 (+16 more)
...
Cohesion: 0.16
Nodes (23): _ai_unconfigured(), build_health_dependencies(), _calendar_library_available(), create_app(), custom_openapi(), _error_response(), HealthDependencies, _live_handler() (+15 more)

### Community 4 - "Child Manager Agent 开发规则"
Cohesion: 0.06
Nodes (47): ApiClient, BffResponse, main(), proxy_request(), Any, register_web(), _require_loopback(), AsyncBaseTransport (+39 more)

### Community 5 - "Phase 1 数据模型：首期一日活动计划完整闭环"
Cohesion: 0.05
Nodes (40): ABC, datetime, AIClientPort, AuditPort, CalendarPort, ClockPort, CryptoPort, DatabaseSessionPort (+32 more)
...
Cohesion: 0.13
Nodes (15): ADR-0009：功能完成前延后生产部署与访问网络决策, 决策, 复审触发条件, 影响, 背景, Child Manager 架构决策记录, 与旧仓库 ADR 的关系, 决策索引 (+7 more)
...
Cohesion: 0.13
Nodes (22): canonical_fingerprint(), idempotency_key_from_fingerprint(), IdempotencyKey, Any, 幂等键最长 200 字符，201 字符必须拒绝。, 重复 query 参数按 (name, value) 元组排序。, test_canonical_fingerprint_basic(), test_canonical_fingerprint_body_stable() (+14 more)
...
### Community 82 - "main codex trae 分支所有权"
Cohesion: 0.11
...
Cohesion: 0.35
Nodes (11): _http_exception_handler(), _request_context_middleware(), FastAPI, StarletteHTTPException, TestClient, test_500_returns_chinese_internal_error(), test_http_exception_400_returns_chinese_envelope(), test_error_responses_include_request_id() (+3 more)
...
### Community 97 - "create-new-feature.sh"
Cohesion: 0.18
...
- **Why does `Child Manager 数据模型设计` connect `14. Word 导出` to `功能规格模板`, `用户故事、需求与可度量结果`, `设置、基础、故事与收尾阶段`, `用户故事任务模板`, `Spec Kit 需求检查清单`, `common.sh`, `10. 后台任务与 AI 结果 Schema`, `12. 系统支撑 Schema`, `Spec Kit 任务转 GitHub Issue`, `按用户故事生成可执行任务`, `规范化 Issue 创建`, `GitHub 远端匹配与任务去重门禁`, `3.8 编辑与 AI 采用`, `15. 安全、隐私与审计`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Why does `Child Manager 系统架构设计` connect `Q: 请阅读以上内容和现有的README.md,AGENTS.md,还有ywyz/kindergartenManager.git的文档，你认为CONTEXT.md需要再加入点什么？` to `教师控制、AI 与 Word 保真`, `setup-tasks.sh`, `歧义与覆盖扫描`, `逐问逐答与增量回写`, `日期校验只软提示`, `确定性与风险相称测试`, `AI Key 加密与数据最小化`, `事实来源冲突时停止固化实现`, `固定 Word 模板完整性`, `common.sh`, `服务、园所与事务边界`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `这是一份word表格` connect `3.7 API 与页面信息架构` to `3.3 数据模型、保留与任务关系`, `common.sh`, `15. 生产部署延后边界`, `4. 架构总览`, `7. API 与契约`, `3.4 加密与密钥`, `3.6 测试与可访问性`?**
