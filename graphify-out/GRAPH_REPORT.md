...
## Corpus Check
- 71 files · ~73,955 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1671 nodes · 1327 edges · 526 communities (121 shown, 405 thin omitted)
...
- [[_COMMUNITY_User Scenarios & Testing (mandatory)|User Scenarios & Testing *(mandatory)*]]
...
- [[_COMMUNITY_Child Manager 20260713 编码前审查报告（Codex + Trae）|Child Manager 20260713 编码前审查报告（Codex + Trae）]]
...
- [[_COMMUNITY_右侧列|右侧列]]
- [[_COMMUNITY_右侧列|右侧列]]
- [[_COMMUNITY_表格第三大行|表格第三大行]]
- [[_COMMUNITY_表格第四行：|表格第四行：]]
...
- [[_COMMUNITY_Q 请阅读以上内容和现有的AGENTS.MD,还有ywyzkindergartenManag|Q: 请阅读以上内容和现有的AGENTS.MD,还有ywyz/kindergartenManag]]
- [[_COMMUNITY_Q 请阅读以上内容和现有的README.md,AGENTS.md,还有ywyzkinderg|Q: 请阅读以上内容和现有的README.md,AGENTS.md,还有ywyz/kinderg]]
- [[_COMMUNITY_Q 请根据现有文档，和旧仓库的文件思考如何撰写 docsPRDlesson-managem|Q: 请根据现有文档，和旧仓库的文件思考如何撰写 docs/PRD/lesson-managem]]
...
- [[_COMMUNITY_Q 请使用graphify update. 进行更新，同时使用子代理进行语义更新，然后思考还|Q: 请使用/graphify update. 进行更新，同时使用子代理进行语义更新，然后思考还]]
...
- [[_COMMUNITY_check-prerequisites.sh script|check-prerequisites.sh script]]
- [[_COMMUNITY_setup-plan.sh script|setup-plan.sh script]]
- [[_COMMUNITY_setup-tasks.sh script|setup-tasks.sh script]]
...
8. `Child Manager 项目上下文` - 15 edges
9. `Child Manager 幼儿园教育管理系统` - 15 edges
...
- `完整 SDD 工作流` --references--> `Spec Kit 实施规划`  [EXTRACTED]
  .specify/workflows/speckit/workflow.yml → .agents/skills/speckit-plan/SKILL.md
- `完整 SDD 工作流` --references--> `Spec Kit 功能规格生成`  [EXTRACTED]
  .specify/workflows/speckit/workflow.yml → .agents/skills/speckit-specify/SKILL.md
- `完整 SDD 工作流` --references--> `Spec Kit 任务生成`  [EXTRACTED]
  .specify/workflows/speckit/workflow.yml → .agents/skills/speckit-tasks/SKILL.md
- `Issue 分支实现独立授权门禁` --semantically_similar_to--> `3. 分支模型与所有权`  [INFERRED] [semantically similar]
  docs/development/m1-issue-drafts.md → docs/development/dual-agent-development.md
...
## Communities (526 total, 405 thin omitted)
...
### Community 1 - "Implementation Plan: 首期一日活动计划完整闭环"
Cohesion: 0.04
...
### Community 2 - "Tasks: 首期一日活动计划完整闭环"
Cohesion: 0.05
...
### Community 5 - "Child Manager Agent 开发规则"
Cohesion: 0.06
Nodes (35): 10. 一日活动计划业务不变量, 11.1 模型访问, 11.2 提示词管理, 11.3 生成行为, 11. AI 与提示词规则, 12. Word 模板与导出, 13. 安全与隐私硬性规则, 14. 审计、日志与错误处理 (+27 more)

### Community 6 - "2026-07-14 只读编码前收敛审查报告"
Cohesion: 0.10
Nodes (32): 2026-07-14 只读编码前收敛审查报告, 分支操作授权门禁, Canonical 阶段状态一致性, 当前不能进入编码, combined-audit.md, 有条件进入 M1 Issue 设计与草拟阶段, 宪章约束已进入计划与任务, CONTEXT.md (+24 more)
...
### Community 8 - "Child Manager 双 Agent 独立开发协议"
Cohesion: 0.08
Nodes (30): 10. 禁止事项, 11. 启动前检查清单, 1. 文档目的, 2. 事实来源与优先级, 3. 分支模型与所有权, 4.1 共享父 Issue, 4.2 实现子 Issue, 4.3 纵向拆分 (+22 more)

### Community 9 - "User Scenarios & Testing *(mandatory)*"
Cohesion: 0.07
...
### Community 12 - "M1 共享父 Issue"
Cohesion: 0.14
Nodes (25): 避免日期专用文档提交, Codex 实现子 Issue, 共同基线分支创建, 冲突冻结, 双 Agent 启动前检查清单, GitHub Issue 创建阶段, graphify 完整性验收, graphify 工作区修改处置 (+17 more)
...
### Community 15 - "ADR-0001：只交付 Cloud 版本，首期单园运行并保留园所隔离边界"
Cohesion: 0.10
...
### Community 16 - "ADR-0007：采用 Caddy、Docker Compose 与文件挂载 Secrets"
Cohesion: 0.10
...
### Community 18 - "Child Manager 20260713 编码前审查报告（Codex + Trae）"
Cohesion: 0.10
Nodes (19): 1. 联合结论, 2.1 文档状态, 2.2 已确认满足的规则, 2. 当前文档与已满足规则, 3.1 模型与契约, 3.2 模板说明, 3.3 状态、证据与分支流程, 3.4 静态验证、图谱、历史与最终基线 (+11 more)
...
### Community 35 - "ADR-0002：采用独立 Web、API、Worker 运行单元的模块化单体"
Cohesion: 0.18
...
### Community 36 - "ADR-0004：采用同源入口、HttpOnly Cookie 与 API 统一授权"
Cohesion: 0.18
...
### Community 37 - "Child Manager PostgreSQL 数据库 Schema"
Cohesion: 0.18
...
- **Why does `教案管理 PRD（首期：一日活动计划）` connect `教案管理 PRD（首期：一日活动计划）` to `12. AI 模型与提示词管理`, `5. 用户与权限`, `7. 首期必要设置`, `8. 核心业务对象`, `9. 日期与上下文规则`, `18. 验收标准`, `11. 计划管理流程`, `README.md`, `14. Word 导出`, `17. 非功能要求`, `10. 教案栏目与格式`, `13. AI 生成流程`, `15. 安全、隐私与审计`, `4. 产品目标`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `Child Manager 数据模型设计` connect `Child Manager 数据模型设计` to `11. 后台任务与 AI 结果`, `16. 外键与删除行为`, `21. 测试与验收`, `4. 模型总览`, `7. AI 模型档案`, `README.md`, `5. 园所与身份模型`, `3. 总体建模原则`, `9. 一日活动计划模型`, `6. 教学设置模型`, `15. 数据快照边界`, `22. 后续扩展边界`, `2. 设计来源与旧系统取舍`, `8. 提示词模型`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `Child Manager 文档交叉审计合并结论` connect `Child Manager 文档交叉审计合并结论` to `3. 二十六项问题的最终结论`, `4. 项目文档更新矩阵`, `README.md`?**
