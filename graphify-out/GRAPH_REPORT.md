...
## Corpus Check
- 68 files · ~71,058 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1340 nodes · 1065 edges · 380 communities (98 shown, 282 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.88)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- [[_COMMUNITY_system-architecture|system-architecture.md]]
...
- [[_COMMUNITY_M0 阻塞项统一修复计划（Codex + Trae）|M0 阻塞项统一修复计划（Codex + Trae）]]
- [[_COMMUNITY_ADR-0005：AI 供应商中立，并建立管理员专用提示词系统|ADR-0005：AI 供应商中立，并建立管理员专用提示词系统]]
...
- [[_COMMUNITY_Specification Quality Checklist 首期一日活动计划完整闭环|Specification Quality Checklist: 首期一日活动计划完整闭环]]
- [[_COMMUNITY_SKILL|SKILL.md]]
- [[_COMMUNITY_SKILL|SKILL.md]]
...
6. `Child Manager 幼儿园教育管理系统` - 15 edges
7. `Tasks: [FEATURE NAME]` - 13 edges
8. `Child Manager 双 Agent 独立开发协议` - 12 edges
9. `Phase 1 数据模型：首期一日活动计划完整闭环` - 12 edges
10. `后台任务状态机与 AI 采用契约` - 11 edges

## Surprising Connections (you probably didn't know these)
- `T003 双实现分支授权` --semantically_similar_to--> `实现分支授权边界`  [INFERRED] [semantically similar]
  specs/001-daily-activity-plan/plan.md → CONTEXT.md
- `M0 complete M1 ready 规格状态` --semantically_similar_to--> `M0 共享设计基线`  [INFERRED] [semantically similar]
  specs/001-daily-activity-plan/spec.md → docs/ROADMAP.md
- `固定 Word 模板保真` --semantically_similar_to--> `M7 固定 Word 导出与历史`  [INFERRED] [semantically similar]
  specs/001-daily-activity-plan/spec.md → docs/ROADMAP.md
...
## Communities (380 total, 282 thin omitted)

### Community 0 - "system-architecture.md"
Cohesion: 0.05
Nodes (43): 供应商中立 AI 适配器, AI 预览采用边界, API 与共享契约, 架构目标, 架构级验证, 同源 Web BFF, 工作日与日期服务, 深模块用例接口 (+35 more)

### Community 1 - "README.md"
Cohesion: 0.06
Nodes (27): ADR-0002：采用独立 Web、API、Worker 运行单元的模块化单体, NiceGUI 一体化进程, React/TypeScript 独立前端, 从首期开始全面微服务化, 决策, 前端直接访问数据库或共享 Repository, 后果, 备选方案 (+19 more)
...
### Community 5 - "首期一日活动计划功能规格"
Cohesion: 0.07
Nodes (34): 实现分支授权边界, Cloud-only 首期范围, 生产部署延后至 M9, 园所数据隔离边界, M0 complete 与 M1 ready, Child Manager 项目上下文, Web API Worker 服务边界, 审计事实来源链 (+26 more)

### Community 6 - "后台任务状态机与 AI 采用契约"
Cohesion: 0.06
Nodes (32): 10. Web 轮询契约, 1. 任务类型, 2. 状态定义, 3. 受理、投递与租约, 4. 模型调用与重试分类, 5. 幂等契约, 6. AI 预览有效性, 7. 各任务输入快照 (+24 more)

### Community 7 - "Phase 3: US1 管理员安全初始化与必要设置"
Cohesion: 0.07
Nodes (31): Phase 10: Polish & Cross-Cutting Concerns（M8 完整验收）, Phase 3: US1 管理员安全初始化与必要设置, Phase 4: US2 教师纯手工教案闭环, Phase 5: US3 管理员配置模型与提示词, Phase 6: US4 教师按栏目使用 AI 并保留决定权, Phase 7: US5 教师处理集体活动原始教案, Phase 8: US6 教师导出并重新下载固定 Word, Phase 9: US7 管理员审计与可降级服务 (+23 more)

### Community 8 - "首期一日活动计划完整闭环任务清单"
Cohesion: 0.07
Nodes (27): 执行授权边界与 RED seam 门禁, Dependencies & Execution Order, 执行授权边界, 第一可用业务 MVP, 首期一日活动计划完整闭环任务清单, 格式, 实现共享基础, Foundational 先写失败测试 (+19 more)
...
Cohesion: 0.09
Nodes (23): AI 提示词管理子系统, AI 生成规则, Child Manager 幼儿园教育管理系统, 一键生成与分栏目生成, 分支协作规范, 后续子系统, 基本规则, 学期、周次与班级 (+15 more)

### Community 12 - "右侧列"
Cohesion: 0.16
Nodes (23): 一日活动反思填写说明, 右侧列, 右侧列为两行：, 填写说明, 左侧列, 文件第一行, 文件第二行, 注意事项： (+15 more)

### Community 13 - "ADR-0001：只交付 Cloud 版本，首期单园运行并保留园所隔离边界"
Cohesion: 0.10
...
### Community 24 - "Execution Steps"
Cohesion: 0.15
Nodes (13): 12. 节假日与日期服务, 14. 可观测性与审计, 16. 性能与扩展, 17. 故障与降级, 18. 测试与架构验证, 19. 关键架构决策摘要, 1. 文档目的, 20. 实施顺序 (+5 more)

### Community 25 - "M0-G1 至 M0-G8 门禁体系"
Cohesion: 0.15
Nodes (13): 内容门禁与操作授权分离, combined-audit 审计事实基线, 最终 docs-only main 共享基线, Child Manager 20260713 编码前审查报告, FR-031 快照原因码与恢复顺序, Git 历史隐私清理, M0 complete, M0-G1 至 M0-G8 门禁体系 (+5 more)

### Community 26 - "Feature Specification: [FEATURE NAME]"
Cohesion: 0.15
Nodes (12): Assumptions, Edge Cases, Feature Specification: [FEATURE NAME], Functional Requirements, Key Entities *(include if feature involves data)*, Measurable Outcomes, Requirements *(mandatory)*, Success Criteria *(mandatory)* (+4 more)

### Community 27 - "ADR-0006：一日活动计划采用固定 Word 模板导出边界"
Cohesion: 0.17
Nodes (11): ADR-0006：一日活动计划采用固定 Word 模板导出边界, UI 直接修改模板并下载, 从零生成 Word 排版, 决策, 只向浏览器返回临时文件, 后果, 备选方案, 复审触发条件 (+3 more)

### Community 28 - "M0 阻塞项统一修复计划（Codex + Trae）"
Cohesion: 0.17
...
### Community 29 - "ADR-0005：AI 供应商中立，并建立管理员专用提示词系统"
Cohesion: 0.18
Nodes (10): Completion Report, Done When, Key rules, Mandatory Post-Execution Hooks, Outline, Phase 0: Outline & Research, Phase 1: Design & Contracts, Phases (+2 more)

### Community 30 - "Child Manager 系统架构设计"
Cohesion: 0.18
Nodes (10): Completion Report, Done When, For AI Generation, Mandatory Post-Execution Hooks, Outline, Pre-Execution Checks, Quick Guidelines, Section Requirements (+2 more)

### Community 31 - "Feature Specification: [FEATURE NAME]"
Cohesion: 0.18
Nodes (10): Checklist Format (REQUIRED), Completion Report, Done When, Mandatory Post-Execution Hooks, Outline, Phase Structure, Pre-Execution Checks, Task Generation Rules (+2 more)

### Community 32 - "ADR-0004：采用同源入口、HttpOnly Cookie 与 API 统一授权"
Cohesion: 0.18
...
### Community 33 - "Child Manager PostgreSQL 数据库 Schema"
Cohesion: 0.18
...
### Community 96 - "7. API 与契约"
Cohesion: 0.67
Nodes (3): 7.1 API 原则, 7.2 跨进程任务契约, 7. API 与契约

### Community 97 - "15. 安全、隐私与审计"
Cohesion: 0.67
...
## Knowledge Gaps
- **932 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+927 more)
...
- **Why does `Child Manager 数据模型设计` connect `Child Manager 数据模型设计` to `README.md`, `3. 总体建模原则`, `9. 一日活动计划模型`, `15. 数据快照边界`, `22. 后续扩展边界`, `2. 设计来源与旧系统取舍`, `5. 园所与身份模型`, `6. 教学设置模型`, `8. 提示词模型`, `11. 后台任务与 AI 结果`, `16. 外键与删除行为`, `4. 模型总览`, `7. AI 模型档案`, `21. 测试与验收`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `教案管理 PRD（首期：一日活动计划）` connect `教案管理 PRD（首期：一日活动计划）` to `17. 非功能要求`, `README.md`, `15. 安全、隐私与审计`, `4. 产品目标`, `18. 验收标准`, `10. 教案栏目与格式`, `13. AI 生成流程`, `5. 用户与权限`, `8. 核心业务对象`, `12. AI 模型与提示词管理`, `7. 首期必要设置`, `9. 日期与上下文规则`, `11. 计划管理流程`, `14. Word 导出`?**
...
- **Should `system-architecture.md` be split into smaller, more focused modules?**
