...
## Corpus Check
- 155 files · ~82,564 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1669 nodes · 1968 edges · 178 communities (163 shown, 15 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 35 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `60990f2b`
...
- `test_500_returns_chinese_internal_error()` --indirect_call--> `_unhandled_error_handler()`  [INFERRED]
  tests/api/test_error_handling.py → apps/api/app.py
- `_runtime_paths()` --calls--> `create_app()`  [EXTRACTED]
  tests/contract/test_openapi_document.py → apps/api/app.py
...
Cohesion: 0.15
Nodes (17): process_export_job(), process_generation_job(), process_retry_job(), build_deterministic_test_broker(), BaseModel, Worker 消息信封：只传 job_id，不携带 API Key、token 或正文。, WorkerMessage, Worker 自动化测试。  使用 StubBroker 验证 broker 工厂、Scheduler 停止语义和 消息只传 job_id 的边界约束。 (+9 more)
...
Cohesion: 0.07
Nodes (55): create_app(), custom_openapi(), _error_response(), _http_exception_handler(), _live_handler(), JSONResponse, Request, _ready_handler() (+47 more)
...
Cohesion: 0.36
Nodes (6): build_redis_broker(), Broker, main(), Broker, Event, serve()
...
- **Why does `Phase 1 数据模型：首期一日活动计划完整闭环` connect `Phase 1 数据模型：首期一日活动计划完整闭环` to `README.md`?**
