...
## Corpus Check
- 156 files · ~84,498 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1741 nodes · 2109 edges · 180 communities (171 shown, 9 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 35 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f6df93cb`
...
3. `create_app()` - 22 edges
4. `教案管理 PRD（首期：一日活动计划）` - 22 edges
5. `Child Manager 系统架构设计` - 22 edges
...
10. `HealthDependencies` - 16 edges
...
- `_make_client()` --calls--> `create_app()`  [EXTRACTED]
  tests/api/test_error_handling.py → apps/api/app.py
...
Cohesion: 0.08
Nodes (29): BaseSettings, Settings, get_db_session(), environment 必须拒绝非 production/development/test 的值。, test_allowed_hosts(), test_cookie_security_non_loopback(), test_cookie_security_production(), test_cookie_security_validation() (+21 more)
...
Cohesion: 0.13
Nodes (14): ApiClient, BffResponse, proxy_request(), Any, register_web(), main(), _require_loopback(), AsyncBaseTransport (+6 more)
...
Cohesion: 0.36
Nodes (6): DeclarativeBase, Base, get_database_url(), Alembic 迁移环境。  模块级代码需兼容两种导入方式： 1. alembic.command.upgrade/downgrade 调用时，context., run_migrations_offline(), run_migrations_online()
...
Cohesion: 0.20
Nodes (6): ABC, AIClientPort, AuditPort, CalendarPort, CryptoPort, RedisPort
...
Cohesion: 0.15
Nodes (12): downgrade(), upgrade(), block_external_network(), isolated_database_url(), _native_psycopg_url(), MonkeyPatch, 返回显式配置的测试数据库 URL。      测试数据库必须在运行前通过环境变量显式指定，缺失时清晰失败，     禁止静默回退到共享或内存数据库，以保证测试隔, require_test_database_url() (+4 more)
...
### Community 73 - "exports.py"
Cohesion: 0.48
Nodes (6): ExportConfirmRequest, ExportListResponse, ExportRecord, ExportRequest, MissingColumnWarning, BaseModel
...
## Knowledge Gaps
- **904 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+899 more)
