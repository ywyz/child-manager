...
## Corpus Check
- 208 files · ~112,449 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2386 nodes · 3695 edges · 209 communities (197 shown, 12 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 147 edges (avg confidence: 0.65)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `39b9764f`
...
## Communities (209 total, 12 thin omitted)
...
Cohesion: 0.06
Nodes (49): ApiClient, BffResponse, proxy_request(), Any, register_web(), _csrf_js(), _js_get(), _nav_html() (+41 more)
...
Cohesion: 0.06
Nodes (39): Config, block_external_network(), isolated_database_url(), migrated_database_url(), _native_psycopg_url(), MonkeyPatch, pytest_configure(), 每个测试前清空内存限流状态，避免跨测试误拦截。 (+31 more)
...
Cohesion: 0.07
Nodes (34): main(), _require_loopback(), BaseSettings, Settings, get_db_session(), Cookie 安全配置校验（Web 与 API 共享）。, 校验 Cookie Secure 与绑定地址的组合是否合法。      仅开发环境允许关闭 Secure；关闭 Secure 时进程必须绑定回环地址。, validate_cookie_security() (+26 more)
...
Cohesion: 0.05
Nodes (59): _build_allowed_origins(), generate_csrf_token(), _is_allowed_origin(), _normalize_origin(), 校验 CSRF Cookie 与 Header，失败时抛 CsrfError。      本函数不依赖 FastAPI/Starlette 的 Request, 把 Origin/Referer 规范化为 scheme://host:port 形式，比较完整同源。, 根据配置生成允许的完整 Origin 集合（scheme + host + effective port）。, 校验签名双提交 CSRF：Cookie 与 Header 必须同时存在、均有效且值相同。 (+51 more)
...
Cohesion: 0.08
Nodes (20): _build_throttle(), deque, InMemoryThrottleBackend, LoginThrottle, Any, datetime, 登录限流器接口：账号级指数退避 + 来源级硬频控。, 判断来源是否触发硬频控（>= 30 次/15 分钟）。 (+12 more)
...
Cohesion: 0.19
Nodes (9): IntegrityError, IdentityService, Any, 幂等确保全局 admin/teacher 角色存在。, Access Token 携带的 family 是否仍活跃。, 身份业务编排：协调 Repository、Token、CSRF 与审计。, parse_refresh_kindergarten_id(), 从 Refresh 明文中解析园所 ID；失败返回 None。 (+1 more)
...
### Community 73 - "app.py"
Cohesion: 0.23
Nodes (19): create_app(), _csrf_error_handler(), custom_openapi(), _error_response(), _http_exception_handler(), _identity_error_handler(), _live_handler(), Exception (+11 more)
...
Cohesion: 0.10
Nodes (30): ConflictError, LastAdminError, LoginResult, 登录成功后的完整结果，避免使用无类型字典。      当前用户对象在事务提交前构造完成，Router 层不再额外查询数据库。, RefreshResult, create_access_token(), decode_access_token(), generate_refresh_value() (+22 more)
...
Cohesion: 0.27
Nodes (8): _convert_varchar36_to_uuid(), _ensure_system_roles(), _migrate_roles_to_global(), reconcile identity schema  Revision ID: 0006_reconcile_identity_schema Revises:, 确保全局 admin/teacher 系统角色存在。, 将旧园所范围的 roles 表收敛为全局只读字典，并更新 user_roles.role_id。, 将指定表的 varchar(36) 列批量转换为 uuid 类型。, upgrade()
...
### Community 199 - "User"
Cohesion: 0.11
Nodes (44): Match, User, CapturedRequest, _click_button(), JsRequestRecorder, Any, NiceGUI 页面级冒烟测试。  使用 nicegui.testing.User 驱动真实页面，验证登录、改密、账号管理和导航 页面渲染及关键交互；通过拦截, 轮询等待指定请求被记录至少 min_count 次。 (+36 more)
...
- **Why does `IdentityService` connect `._record_identity` to `tokens.py`, `m2-issue-drafts.md`, `test_users_contract.py`, `test_session_boundaries.py`, `init_admin.py`, `_seed_two_admins`, `test_auth_smoke.py`, `AuditService`, `auth.py`, `validate_password`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `User` connect `User` to `select`, `tokens.py`, `m2-issue-drafts.md`, `csrf.py`, `IdentityRepository`, `._record_identity`, `models.py`, `validate_password`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `IdentityRepository` connect `IdentityRepository` to `select`, `tokens.py`, `3.5 API、任务状态与幂等`, `csrf.py`, `_seed_two_admins`, `AuditService`, `validate_password`, `repository.py`?**
