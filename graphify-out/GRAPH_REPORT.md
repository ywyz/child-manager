...
## Summary
- 1827 nodes · 1400 edges · 607 communities (128 shown, 479 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 14 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f1a4d1ee`
...
1. `Child Manager Agent 开发规则` - 25 edges
2. `Child Manager 数据模型设计` - 24 edges
3. `教案管理 PRD（首期：一日活动计划）` - 22 edges
4. `Child Manager 系统架构设计` - 22 edges
5. `Child Manager 幼儿园教育管理系统` - 19 edges
6. `Child Manager PostgreSQL 数据库 Schema` - 19 edges
7. `Child Manager 产品与工程路线图` - 18 edges
8. `Child Manager 项目上下文` - 17 edges
9. `Tasks: 首期一日活动计划完整闭环` - 17 edges
10. `Tasks: [FEATURE NAME]` - 13 edges

## Surprising Connections (you probably didn't know these)
- `共同实施路线` --semantically_similar_to--> `M0 至 M9 里程碑依赖链`  [INFERRED] [semantically similar]
  CONTEXT.md → docs/ROADMAP.md
- `Cloud-only 三服务架构` --semantically_similar_to--> `Web API Worker 服务边界`  [INFERRED] [semantically similar]
  README.md → AGENTS.md
- `生产部署延后至 M9` --semantically_similar_to--> `生产部署延后`  [INFERRED] [semantically similar]
  docs/development/local-development-environments.md → AGENTS.md
- `先 RED 后实现的执行顺序` --implements--> `Child Manager Agent 开发规则`  [INFERRED]
  specs/001-daily-activity-plan/tasks.md → AGENTS.md
- `Child Manager 幼儿园教育管理系统` --references--> `产品与工程路线图`  [EXTRACTED]
...
## Communities (607 total, 479 thin omitted)
...
Cohesion: 0.05
Nodes (40): 10. 一日活动计划业务不变量, 11.1 模型访问, 11.2 提示词管理, 11.3 生成行为, 11. AI 与提示词规则, 12. Word 模板与导出, 13. 安全与隐私硬性规则, 14. 审计、日志与错误处理 (+32 more)
...
Cohesion: 0.06
Nodes (32): 10. 当前共同下一步, 11.1 文档职责与工具基线, 11. 高风险点, 12. 系统架构基线, 13. 验证基线, 14. CONTEXT 更新规则, 1. 本文档的用途, 2. 固定阅读顺序 (+24 more)
...
Cohesion: 0.14
Nodes (14): AI 生成规则, Child Manager 幼儿园教育管理系统, Word 导出, 一键生成与分栏目生成, 分支协作规范, 后续子系统, 异步任务与反馈, 总体架构 (+6 more)
...
### Community 42 - "API v1 契约总则"
Cohesion: 0.22
Nodes (10): AI 预览与教师采用, 一日活动计划业务不变量, 外部网络确定性测试替身, 下载加速不等于外部服务连通性, API v1 契约总则, Cookie CSRF 会话契约, 幂等异步任务契约, 乐观锁与预览失效校验 (+2 more)

### Community 43 - "双实现本地开发环境"
Cohesion: 0.22
Nodes (10): 生产部署延后, Web API Worker 服务边界, 双实现本地开发环境, 本地服务仅绑定回环地址, PostgreSQL 与 Redis 最小 Compose 依赖, 生产部署延后至 M9, 固定本地档位与运行时隔离, Codex 与 Trae 工作树隔离 (+2 more)
...
Cohesion: 0.25
Nodes (8): AI 提示词管理子系统, 固定 Word 模板导出, 认证与用户 API, Child Manager OpenAPI v1, 教案任务与导出 API, 设置与提示词 API, 首期实现与验收合同, 浏览器端到端验收路径
...
### Community 55 - "技术栈与中国大陆网络边界"
Cohesion: 0.29
Nodes (7): CERNET 用户级 uv 镜像, 容器镜像 shell 变量覆盖, 锁定的 Python 质量门禁工具链, 锁定官方容器镜像默认值, 官方 PyPI 临时回退, 当前应用无需 npm 或 Node.js 安装链, 技术栈与中国大陆网络边界
...
### Community 66 - "M0 至 M9 里程碑依赖链"
Cohesion: 0.33
Nodes (6): 共同实施路线, 工作树端口数据运行时隔离, M1 工程骨架与质量基线, M6 AI 异步生成与人工采用, M0 至 M9 里程碑依赖链, 产品与工程路线图
...
### Community 82 - "双 Agent 独立开发协议"
Cohesion: 0.40
Nodes (5): main codex trae 分支所有权, 双 Agent 独立开发协议, 只读交叉评审, M1 工程骨架与质量基线 Issue 草稿, 共享父 Issue 与实现子 Issue 模型
...
Cohesion: 0.40
Nodes (5): 基本规则, 学期、周次与班级, 工作日校验, 查找与浏览, 首期范围：一日活动计划
...
### Community 128 - "用户与权限"
Cohesion: 0.67
Nodes (3): 教师, 用户与权限, 管理员

## Knowledge Gaps
- **1315 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+1310 more)
...
- **Why does `这是一份word表格` connect `这是一份word表格` to `右侧列为两行：`, `README.md`, `右侧列`, `右侧列`, `右侧列`, `右侧列`, `右侧列为两行：`?**
