# Graph Report - child-manager-docs.AOqKdv  (2026-07-21)

## Corpus Check
- 74 files · ~77,210 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 757 nodes · 225 edges · 546 communities (19 shown, 527 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ce4a7504`
...
1. `ADR-0001：只交付 Cloud 版本，首期单园运行并保留园所隔离边界` - 9 edges
2. `正文` - 9 edges
3. `Child Manager 单实现开发协议` - 9 edges
4. `Child Manager 一日活动计划 OpenAPI 3.1` - 9 edges
5. `Child Manager 贡献指南` - 7 edges
6. `ADR-0002：采用独立 Web、API、Worker 运行单元的模块化单体` - 7 edges
7. `ADR-0003：PostgreSQL 保存任务权威状态，Dramatiq 与 Redis 负责异步执行` - 7 edges
8. `备选方案` - 7 edges
9. `ADR-0004：采用同源入口、HttpOnly Cookie 与 API 统一授权` - 7 edges
10. `ADR-0005：AI 供应商中立，并建立管理员专用提示词系统` - 7 edges
...
## Communities (546 total, 527 thin omitted)

### Community 0 - "Child Manager 产品与工程路线图"
Cohesion: 0.11
Nodes (19): 0. 草稿事实与授权边界, 2. Codex 实现子 Issue 草稿, 3. Trae 实现子 Issue 草稿, 4. 草稿只读复核清单, M2 认证、授权与身份审计 Issue 草稿与执行记录, T021～T035 有序执行清单, T021～T035 有序执行清单, 关联与边界 (+11 more)

### Community 1 - "M0 阻塞项统一修复执行方案（Codex + Trae）"
Cohesion: 0.18
Nodes (9): 1. 目的, 2. 事实来源, 3. 分支模型, 4. 文档流程, 5. Issue 驱动实现, 6. Codex 实现, 7. Review 与发布, 8. 历史双实现退役 (+1 more)

### Community 2 - "Child Manager 2026-07-14 编码前审查解决方案"
Cohesion: 0.18
Nodes (11): 1. 共享父 Issue 草稿, 任务映射与执行顺序, 入口门禁, 共享事实来源, 共同出口门禁, 共同目标, 实现子 Issue, 当前状态与入口 (+3 more)

### Community 3 - "Tasks: 首期一日活动计划完整闭环"
Cohesion: 0.25
Nodes (7): 1. 开始工作前, 2. 分支职责, 3. Design → Implement, 4. Issue 要求, 5. 提交、验证与 Review, 6. 发布与历史, Child Manager 贡献指南

### Community 4 - "Child Manager Agent 开发规则"
Cohesion: 0.29
Nodes (6): 1. 决策, 2. 不变目标, 3. 迁移基线, 4. Trae 归档, 5. M2 Issue, Repository Workflow Reset（2026-07-21）
...
## Knowledge Gaps
- **550 isolated node(s):** `check-prerequisites.sh script`, `common.sh script`, `create-new-feature.sh script`, `setup-plan.sh script`, `setup-tasks.sh script` (+545 more)
...
- **Why does `M2 认证、授权与身份审计 Issue 草稿与执行记录` connect `Child Manager 产品与工程路线图` to `M0 阻塞项统一修复执行方案（Codex + Trae）`, `Child Manager 2026-07-14 编码前审查解决方案`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `ADR-0001：只交付 Cloud 版本，首期单园运行并保留园所隔离边界` connect `Child Manager 幼儿园教育管理系统` to `README.md`?**
