...
## Corpus Check
- 375 files · ~225,671 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3494 nodes · 10501 edges · 285 communities (198 shown, 87 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 906 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `37919f72`
...
1. `ActorFixture` - 207 edges
2. `IdentityError` - 182 edges
3. `csrf_headers()` - 171 edges
4. `SessionUser` - 170 edges
5. `ContractModel` - 154 edges
...
Cohesion: 0.14
Nodes (10): AuditRepository, UUID, InvitationRecord, IdentityService, ManagedUser, datetime, UUID, test_identity_audit_rejects_authentication_material_hidden_in_allowed_fields() (+2 more)
...
Cohesion: 0.10
Nodes (25): ai_adoption_service(), AiAdoptionService, _native_url(), preview_is_current(), AiTaskCode, Any, datetime, JsonValue (+17 more)
...
Cohesion: 0.15
Nodes (15): AgeGroupRecord, AreaInput, AreaRecord, ClassRecord, KindergartenRecord, date, UUID, 园所范围设置的 PostgreSQL Repository。 (+7 more)
...
Cohesion: 0.09
Nodes (26): AiExecutionContext, AiJobAuthorizer, AiJobStore, AiJobStoreProtocol, _log_sanitized_exception(), Any, datetime, Exception (+18 more)

### Community 10 - "AI Schemas"
Cohesion: 0.37
Nodes (33): EditableContent, PromptSpec, AiAreaGame, AiDailyReflection, AiGroupActivity, AiMorningActivity, AiMorningTalk, AreaGame (+25 more)
...
Cohesion: 0.12
Nodes (21): ai_generation_service(), ai_model_service(), ai_retry_service(), authenticated_session(), HealthDependencies, IdentityServiceDependency, reflection_generation_service(), settings_service() (+13 more)

### Community 13 - "AI Job Actors"
Cohesion: 0.10
Nodes (32): Actor, AiJobScopeResolver, AiRunner, build_ai_job_runner(), build_ai_result_repository(), build_prompt_test_executor(), build_worker_scope_resolver(), _native_url() (+24 more)

### Community 14 - "AI Key Rotation"
Cohesion: 0.18
Nodes (18): admin_session(), current_session(), AuthenticatedSessionDependency, CurrentSessionDependency, TestClient, test_open_requires_current_semester(), test_teacher_list_hides_plans_when_the_associated_class_is_inactive(), test_unassociated_admin_can_view_and_archive_but_cannot_edit_body() (+10 more)

### Community 15 - "Jobs Repository"
Cohesion: 0.15
Nodes (26): JobMessage, Redis 中唯一允许传递的最小任务消息。, SimpleNamespace, CountingClient, _executor(), Any, datetime, LogCaptureFixture (+18 more)
...
Cohesion: 0.12
Nodes (27): backup_auth_api_request(), backup_login_api_request(), backup_reauthentication_api_request(), NiceGUI 服务端 BFF 客户端的公开接缝。, 以请求正文提交两项备用因素，不把秘密放入 URL。, 为当前备用会话取得仅可新增通行密钥的短时证明。, 读取本人最近 20 条内建安全事件，不产生已读状态。, 从浏览器经同源 BFF 调用 API，并为写请求取得 CSRF token。 (+19 more)

### Community 18 - "Test Helpers"
Cohesion: 0.12
Nodes (43): provision_enabled_ai_model(), provision_editable_plan_context(), date, TestClient, TestClient, test_generation_reject_adopt_and_retry_write_sanitized_audit_rows(), _configure_batch_areas(), _idempotent_headers() (+35 more)
...
Cohesion: 0.14
Nodes (25): IntegrityError, NoReturn, SessionUser, current_semester_selection_is_valid(), lead_teacher_selection_is_valid(), _native_url(), normalize_class_areas(), _normalize_display_name() (+17 more)

### Community 30 - "Calendar Client"
Cohesion: 0.07
Nodes (46): UUID, run_rotation(), _aad(), AiKeyEnvelope, decrypt_api_key(), decrypt_api_key_with_provider(), encrypt_api_key(), encrypt_api_key_with_provider() (+38 more)
...
Cohesion: 0.11
Nodes (24): _calendar_library_available(), map_timor_payload(), AsyncBaseTransport, date, TimorWorkdayClient, WorkdayResult, Any, date (+16 more)
...
Cohesion: 0.14
Nodes (13): AuthorRecord, LessonPlanRepository, _plan(), PlanCreationContext, PlanRecord, Any, date, UUID (+5 more)

### Community 35 - "AI Client Retry"
Cohesion: 0.12
Nodes (16): AiClientError, RuntimeError, AddStepStore, AlwaysTimeoutClient, InvalidAddStepClient, Any, datetime, UUID (+8 more)
...
Cohesion: 0.10
Nodes (24): build_test_broker(), Any, 生产 Redis 与确定性测试消息代理装配。, 向已注册 actor 投递唯一的 job_id。, RedisJobDispatcher, AiJobRetry, RuntimeError, 通知消息代理按权威任务给出的退避时间重投。 (+16 more)

### Community 46 - "Prompt Test Store"
Cohesion: 0.25
Nodes (11): lesson_plan_service(), LessonPlanService, OpenPlanResult, PlanView, _PlanViewSeed, date, UUID, 完成单一用例响应；外网解析发生在业务事务关闭之后。 (+3 more)
...
Cohesion: 0.12
Nodes (26): login_page_text(), users_page_text(), BrowserContext, candidate_totp_counters(), _counter(), generate_totp(), _hotp(), RFC 6238 TOTP 原语；持久化重放保护由 Repository 完成。 (+18 more)
...
Cohesion: 0.07
Nodes (40): job_query_service(), JobStatus, BatchJobAggregationRepository, Any, UUID, `ai.batch` 父任务的只读状态投影。, 从恰好四个子任务派生父任务响应，不写入父任务执行字段。, JobQueryService (+32 more)

### Community 54 - "API Startup Security Checks"
Cohesion: 0.20
Nodes (9): lesson_plan_source_service(), LessonPlanSourceRecord, LessonPlanSourceRepository, LessonPlanSourceService, UUID, 保留可显示文件名，但绝不保存路径、控制字符或绝对位置。, 所有来源查询和写入均以 kindergarten_id 为首个范围条件。, _record() (+1 more)
...
Cohesion: 0.25
Nodes (5): MonkeyPatch, test_rendered_plan_editor_has_labelled_status_fields_focus_order_and_touch_targets(), MonkeyPatch, test_editor_input_debounces_autosave_and_archive_immediately_disables_fields(), test_plan_home_renders_real_views_and_all_frozen_filters()
...
Cohesion: 0.32
Nodes (14): create_app(), check(), dependencies(), Path, test_database_failure_returns_stable_503_code(), test_default_dependencies_check_real_local_runtime(), test_each_optional_dependency_only_degrades_ready_response(), test_framework_http_exception_uses_error_response() (+6 more)
...
Cohesion: 0.16
Nodes (14): AiKeyProvider, AiModelService, _display(), _key(), _native_url(), Resolver, UUID, AI 模型档案生命周期与调用配置 revision 事务。 (+6 more)
...
Cohesion: 0.21
Nodes (9): ChallengePurpose, StrEnum, _credential(), CredentialRecord, _challenge_digest(), _client_challenge(), _decode_base64url(), Any (+1 more)
...
Cohesion: 0.09
Nodes (30): normalize_phone(), normalize_username(), hash_password(), password_needs_rehash(), password_violations(), Path, verify_password(), _weak_passwords() (+22 more)
...
Cohesion: 0.18
Nodes (9): AiGroupActivityStep, DailyReflection, GroupActivityStepCandidate, LessonPlanReference, _require_nonblank(), _require_question(), _require_statement(), _validate_group_add_step_index() (+1 more)

### Community 76 - "Prompt Test API Tests"
Cohesion: 0.23
Nodes (7): GroupActivityJobService, Any, 保持拆分与新增环节相互独立，且只接受已确认、已保存的输入。, 新增环节只基于教师已采用并保存的完整当前集体活动。, GroupActivityStep, 按任务冻结的过程长度校验索引；越界必须进入结构错误重试。, validate_group_add_step_result()
...
Cohesion: 0.28
Nodes (14): hash_refresh_token(), Access JWT 与 opaque Refresh token 接缝。, _base64url(), _insert_credential(), _native_url(), MonkeyPatch, TestClient, UUID (+6 more)
...
Cohesion: 0.24
Nodes (17): identity_service(), _authentication_credential(), _base64url(), _identity_service(), _native_url(), MonkeyPatch, TestClient, _registration_credential() (+9 more)
...
Cohesion: 0.33
Nodes (10): AiRecoveryStore, AiResultMaintenanceCounts, AiResultMaintenanceRepository, datetime, Protocol, UUID, 逐园执行预览过期和正文清理，并只记录计数。, 从 PostgreSQL 预留可恢复任务后逐项投递，单项失败不阻断后续任务。 (+2 more)
...
Cohesion: 0.08
Nodes (40): append_ai_event(), Any, UUID, _native_url(), UUID, Broker, UUID, 仅投递 job_id 的提示词测试分发边界。 (+32 more)
...
Cohesion: 0.13
Nodes (29): pytest, admin_client(), passkey_client(), MonkeyPatch, TestClient, 通过 FastAPI 身份依赖注入建立已 step-up 管理员，不借用密码登录。, _event(), test_ai_audit_event_codes_cover_creation_retries_result_reject_and_adopt() (+21 more)
...
Cohesion: 0.47
Nodes (14): _complete_preview(), _headers(), _native_url(), _prepare_adopted_split(), Any, TestClient, UUID, _request_generation() (+6 more)
...
Cohesion: 0.27
Nodes (12): _button(), Any, MonkeyPatch, T125 集体活动两阶段 NiceGUI 冒烟 RED。, test_failed_add_preserves_split_and_added_step_marker_can_be_cleared(), test_source_confirmation_then_adopted_split_enables_add_step(), _job(), _plan() (+4 more)
...
Cohesion: 0.09
Nodes (25): plan_api_request(), 只通过同源 BFF 访问教案及其任务端点。, AiSectionAction, preview_title(), 教案 AI 预览的稳定栏目、动作与展示数据。, ai_job_status(), AiJobStatus, poll_interval_ms() (+17 more)
...
Cohesion: 0.40
Nodes (10): _assert_complete_split(), _confirmed_source(), _jobs(), Any, T122 集体活动拆分与新增环节 Worker RED。, _service(), test_add_step_has_independent_preview_and_retries_only_a_real_failed_add_without_rollback(), test_add_step_requires_saved_adoption_and_freezes_the_current_group_activity() (+2 more)
...
Cohesion: 0.43
Nodes (7): _contract(), Any, US5 集体活动来源与两阶段 AI 契约 RED。, _source_payload(), test_source_metadata_is_closed_and_never_exposes_original_text_or_attachment(), test_source_page_is_closed_and_preserves_pagination_metadata(), test_split_and_incremental_add_schemas_are_closed_and_validate_index_bounds()
...
- **Why does `IdentityError` connect `AI Generation Service` to `Identity & Audit`, `Auth Endpoints`, `Job Recovery`, `AI Job Actors`, `WebAuthn Challenges`, `test_ai_adoption_service.py`, `User Management`, `Settings Repository`, `Lesson Plan Repository`, `Prompt Repository`, `TOTP Encryption`, `Prompt Test Store`, `FastAPI Application`, `Jobs API`, `API Startup Security Checks`, `Backup Maintenance Tests`, `App Health Tests`, `AI Client & SSRF`, `Job Status UI`, `Identity Helpers`, `Admin Initialization`, `AI Generation Service Tests`, `Recovery Tests`, `Prompt Test Dispatch`, `Auth Logic`, `Content Schemas`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `ActorFixture` connect `Backup Enrollment Tests` to `Identity & Audit`, `Auth Throttling`, `AI Key Rotation`, `test_ai_batch_generation.py`, `WebAuthn Tests`, `Jobs Repository`, `Test Helpers`, `AI Batch Tests`, `test_ai_adoption_service.py`, `AI Generation Tests`, `Calendar Client`, `TOTP Encryption`, `Backup Maintenance Tests`, `AI Preview Lifecycle Tests`, `AI Generation Service Tests`, `Invitation Tests`, `Token Management`, `Recovery Tests`, `Content Schemas`, `Canonical Fingerprint`?**
  _High betweenness centrality (0.053) - this node is a cross-community bridge._
- **Why does `SessionUser` connect `Settings Repository` to `Identity Repository`, `Identity & Audit`, `Auth Endpoints`, `Job Recovery`, `AI Generation Service`, `AI Key Rotation`, `WebAuthn Challenges`, `test_ai_adoption_service.py`, `Lesson Plan Repository`, `Prompt Repository`, `TOTP Encryption`, `Prompt Test Store`, `Jobs API`, `API Startup Security Checks`, `AI Client & SSRF`, `Job Status UI`, `Identity Helpers`, `Admin Initialization`, `AI Generation Service Tests`, `Prompt Test Dispatch`, `Auth Logic`, `Content Schemas`?**
...
- **Are the 44 inferred relationships involving `IdentityError` (e.g. with `create_app()` and `HealthDependencies`) actually correct?**
  _`IdentityError` has 44 INFERRED edges - model-reasoned connections that need verification._
- **Are the 33 inferred relationships involving `SessionUser` (e.g. with `HealthDependencies` and `AuditRepository`) actually correct?**
  _`SessionUser` has 33 INFERRED edges - model-reasoned connections that need verification._
- **Are the 140 inferred relationships involving `ContractModel` (e.g. with `AiGenerationAuditMetadata` and `AuditEventReference`) actually correct?**
  _`ContractModel` has 140 INFERRED edges - model-reasoned connections that need verification._