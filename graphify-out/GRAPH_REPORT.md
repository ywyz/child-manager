...
## Corpus Check
- 205 files · ~107,127 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2260 nodes · 3372 edges · 205 communities (193 shown, 12 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 123 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `07e5869d`
...
1. `IdentityService` - 68 edges
2. `IdentityRepository` - 45 edges
3. `create_app()` - 34 edges
4. `CurrentUser` - 30 edges
5. `HealthDependencies` - 27 edges
6. `Settings` - 25 edges
...
Cohesion: 0.05
Nodes (61): main(), _require_loopback(), BaseSettings, EventDict, Settings, configure_logging(), 结构化日志配置。  调用共享脱敏模块，确保日志中的敏感字段 （密钥、密码、令牌、Cookie 等）在输出前被脱敏。, 配置 structlog 处理链，包含脱敏处理器。 (+53 more)
...
### Community 26 - "app.py"
Cohesion: 0.19
Nodes (25): check_csrf(), get_current_user(), Depends, Request, Session, 从 FastAPI Request 提取 CSRF 所需字段并校验。, _build_current_user(), change_password() (+17 more)
...
Cohesion: 0.17
Nodes (18): ChangePasswordRequest, CsrfResponse, LoginRequest, BaseModel, 校验角色列表：非空、唯一、仅允许 admin/teacher。, RefreshRequest, ResetPasswordRequest, UserPage (+10 more)
...
### Community 42 - "ADR-0006：一日活动计划采用固定 Word 模板导出边界"
Cohesion: 0.11
...
Cohesion: 0.20
Nodes (10): UserCreateRequest, BaseModel, _schema_fields(), test_reset_password_request_has_new_password(), test_user_create_request_has_identity_fields(), test_user_create_request_rejects_duplicate_roles(), test_user_create_request_rejects_unknown_roles(), test_user_create_request_requires_at_least_one_role() (+2 more)
...
Cohesion: 0.19
Nodes (8): AuditEvent, AuditRepository, Any, AuditService, Any, 业务审计门面，隐藏 Repository 细节并约束身份审计字段。, 记录身份审计事件，元数据仅保留来源 IP。, Session

### Community 52 - "ADR-0004：采用同源入口、HttpOnly Cookie 与 API 统一授权"
Cohesion: 0.20
Nodes (6): LastAdminError, IdentityService, Access Token 携带的 family 是否仍活跃。, 身份业务编排：协调 Repository、Token、CSRF 与审计。, M2 单园部署：返回唯一园所。          当前仅支持单园，所有已认证请求通过 token/session 携带明确的         kindergar, 幂等确保园所具备 admin/teacher 角色。
...
### Community 55 - "DatabaseSessionPort"
Cohesion: 0.26
...
Cohesion: 0.18
Nodes (4): ForbiddenError, IdentityError, Exception, 身份域错误基类，携带稳定错误码、中文消息与 HTTP 状态码。
...
Cohesion: 0.20
Nodes (14): Any, create_access_token(), decode_access_token(), generate_refresh_value(), Any, Access JWT 与 opaque Refresh 令牌。, 签发 Access JWT。      family_id 与 Refresh Token family 绑定，用于在退出、改密、重置、停用或     Refr, 生成 opaque Refresh 明文；前缀编码园所 ID 以便 Repository 显式隔离。 (+6 more)
...
Cohesion: 0.32
Nodes (20): get_db(), require_admin(), activate_user(), create_user(), deactivate_user(), get_user(), list_users(), Depends (+12 more)

### Community 185 - "models.py"
Cohesion: 0.24
Nodes (8): DeclarativeBase, _now(), datetime, Base, get_database_url(), Alembic 迁移环境。  模块级代码需兼容两种导入方式： 1. alembic.command.upgrade/downgrade 调用时，context., run_migrations_offline(), run_migrations_online()

### Community 186 - "normalize_username"
Cohesion: 0.24
Nodes (10): normalize_phone(), normalize_username(), 返回规范化用户名：NFKC、去首尾空白、小写。, 返回 E.164 手机号或空值；当前只接受中国大陆手机号。, verify_password(), parse_refresh_kindergarten_id(), 从 Refresh 明文中解析园所 ID；失败返回 None。, test_normalize_phone_e164_or_empty() (+2 more)

### Community 187 - "select"
Cohesion: 0.17
...
Cohesion: 0.17
Nodes (14): _default_password_input(), _has_initial_admin(), 交互式终端使用 getpass 避免回显；非 TTY 环境回退到 input 以支持自动化测试。, _read_password(), run_init_admin(), hash_password(), 校验密码复杂度：长度 15–128 字符且不在常见弱密码列表中。, validate_password() (+6 more)

### Community 191 - "User"
Cohesion: 0.15
Nodes (8): Kindergarten, _now(), datetime, RefreshToken, datetime, 撤销单个 Refresh Token（用于正常轮换）。, 撤销整个 Refresh family（用于退出、改密、重置、停用、重放）。, 撤销用户的全部 Refresh Token 及其 family。
...
### Community 195 - "env.py"
Cohesion: 0.27
Nodes (11): _audit_count(), _commit(), kindergarten(), MonkeyPatch, 审计持久化测试：关键身份事件在失败路径下仍写入 audit_events。, 在已迁移的隔离 schema 上构造 IdentityService。, service(), _set_keys() (+3 more)

### Community 197 - "Role"
Cohesion: 0.13
Nodes (6): Role, 返回具有 admin 角色的有效用户数量。, 在指定园所内通过 token_hash 定位 Refresh Token。          Refresh 明文已编码园所 ID，调用方解析后必须显式传入 k, 带行锁的 find_refresh_token_by_hash；隔离语义同上。, family 内存在未撤销的 token 时视为活跃。, select
...
### Community 199 - "._db"
Cohesion: 0.33
Nodes (9): User, KindergartenSnapshot, _add_js_rules(), NiceGUI 页面级冒烟测试。  使用 nicegui.testing.User 驱动真实页面，验证登录、改密、账号管理和导航 页面渲染及关键元素存在；页面内, 为页面中 run_javascript 发起的 API 调用注入 mock 响应。, test_change_password_page_renders_form(), test_login_page_renders_form(), test_navigation_renders_header_and_links() (+1 more)

### Community 200 - "hash_refresh_value"
Cohesion: 0.22
Nodes (10): hash_refresh_value(), 返回 Refresh 明文 SHA-256 哈希。, kindergarten(), MonkeyPatch, Refresh Token family 语义测试。, 在已迁移的隔离 schema 上构造 IdentityService。, service(), _set_keys() (+2 more)

### Community 201 - "UserPatch"
Cohesion: 0.29
Nodes (4): username/display_name 只能作为有效字符串提交，不允许显式设为 null。, phone_e164 是否在请求中被显式提供（包括设为 null）。, UserPatch, Self

### Community 202 - "dependencies.py"
Cohesion: 0.53
...
- **Why does `IdentityService` connect `ADR-0004：采用同源入口、HttpOnly Cookie 与 API 统一授权` to `app.py`, `env.py`, `test_alembic_bootstrap.py`, `._db`, `hash_refresh_value`, `UserPatch`, `dependencies.py`, `后台任务状态机与 AI 采用契约`, `Settings`, `proxy_request`, `ADR-0002：采用独立 Web、API、Worker 运行单元的模块化单体`, `CurrentUser`, `HealthDependencies`, `normalize_username`, `session.py`?**
...
- **Are the 4 inferred relationships involving `CurrentUser` (e.g. with `HealthDependencies` and `IdentityService`) actually correct?**
  _`CurrentUser` has 4 INFERRED edges - model-reasoned connections that need verification._
- **Are the 3 inferred relationships involving `HealthDependencies` (e.g. with `ForbiddenError` and `UnauthenticatedError`) actually correct?**
  _`HealthDependencies` has 3 INFERRED edges - model-reasoned connections that need verification._