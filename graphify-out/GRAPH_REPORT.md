...
## Summary
- 3181 nodes · 9160 edges · 276 communities (189 shown, 87 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 799 edges (avg confidence: 0.54)
- Token cost: 13,287 input · 8,055 output

## Graph Freshness
- Built from commit: `48fb39e6`
...
## Communities (276 total, 87 thin omitted)
...
Cohesion: 0.17
Nodes (8): AuditRepository, IdentityService, ManagedUser, UUID, SessionUser, Dispatcher, Protocol, BackupAuthenticationStatus
...
Cohesion: 0.14
Nodes (27): Event, JobMessage, Redis 中唯一允许传递的最小任务消息。, SimpleNamespace, CountingClient, _executor(), Any, datetime (+19 more)
...
Cohesion: 0.20
Nodes (9): UUID, IdentityAuditEventCode, IdentityAuditMetadata, StrEnum, 身份阶段的稳定审计事件代码与最小资源引用。, 身份审计只允许承载最小、严格类型化的非秘密元数据。, test_identity_audit_rejects_authentication_material_hidden_in_allowed_fields(), test_identity_audit_rejects_sensitive_metadata_keys() (+1 more)
...
Cohesion: 0.28
Nodes (38): EditableContent, PromptSpec, BaseModel, 固定提示词目录、输入与结果 Schema 路由。, _spec(), AiAreaGame, AiDailyReflection, AiGroupActivity (+30 more)

### Community 11 - "Auth & CSRF"
Cohesion: 0.05
Nodes (52): _allowed_origins(), _loopback_aliases(), 同源 Cookie、WebAuthn、邀请、恢复与会话端点。, _encode(), issue_csrf_token(), verify_csrf_token(), AdminCredentialRevocationResult, AuthenticationCredential (+44 more)

### Community 12 - "AI Generation Service"
Cohesion: 0.33
Nodes (4): AiGenerationAcceptance, AiGenerationService, Any, UUID

### Community 13 - "AI Job Actors"
Cohesion: 0.09
Nodes (35): Actor, AiJobScopeResolver, AiRunner, build_ai_job_runner(), build_ai_result_repository(), build_prompt_test_executor(), build_worker_scope_resolver(), _native_url() (+27 more)

### Community 14 - "AI Key Rotation"
Cohesion: 0.09
Nodes (36): AiKeyProvider, UUID, run_rotation(), _aad(), AiKeyEnvelope, decrypt_api_key(), decrypt_api_key_with_provider(), encrypt_api_key() (+28 more)
...
Cohesion: 0.09
Nodes (37): current_session(), AuthenticatedSessionDependency, pytest, admin_client(), passkey_client(), MonkeyPatch, TestClient, 通过 FastAPI 身份依赖注入建立已 step-up 管理员，不借用密码登录。 (+29 more)
...
Cohesion: 0.10
Nodes (35): ChallengeBinding, ChallengeRecord, consume_challenge(), issue_challenge(), IssuedChallenge, datetime, WebAuthn ceremony challenge 的公共领域 seam。, 签发绑定上下文、五分钟有效且只保存摘要的 challenge。 (+27 more)
...
Cohesion: 0.18
Nodes (25): create_completed_ai_preview(), provision_enabled_ai_model(), TestClient, UUID, _idempotent_headers(), TestClient, test_batch_accepts_exactly_four_independent_children_and_derives_parent(), test_batch_database_parent_is_never_executable_or_dispatched() (+17 more)
...
Cohesion: 0.14
Nodes (18): map_timor_payload(), AsyncBaseTransport, date, TimorWorkdayClient, WorkdayResult, 园所范围工作日缓存 Repository。, combine_workday_results(), date (+10 more)
...
Cohesion: 0.12
Nodes (23): identity_service(), _aad(), decrypt_totp_secret(), decrypt_totp_secret_with_provider(), encrypt_totp_secret(), encrypt_totp_secret_with_provider(), FileIdentitySecretKeyProvider, Path (+15 more)
...
Cohesion: 0.33
Nodes (11): authentication_verify(), _clear_auth_cookies(), _cookie_secure(), csrf(), me(), _payload(), Response, SettingsServiceDependency (+3 more)

### Community 45 - "Job Dispatcher"
Cohesion: 0.14
Nodes (19): Any, 向已注册 actor 投递唯一的 job_id。, RedisJobDispatcher, AiJobRetry, RuntimeError, 通知消息代理按权威任务给出的退避时间重投。, _ai_actor(), FakeRunner (+11 more)
...
Cohesion: 0.19
Nodes (12): _auth_throttle(), _error_response(), FastAPI, Request, UUID, FastAPI 应用装配、统一异常转换与健康端点。, _request_id(), Docker Compose 开发环境配置 (+4 more)
...
Cohesion: 0.18
Nodes (13): get_job(), AdminSessionDependency, UUID, JobQueryServiceDependency, JobStatus, BatchJobAggregationRepository, UUID, `ai.batch` 父任务的只读状态投影。 (+5 more)
...
Cohesion: 0.12
Nodes (21): normalize_phone(), normalize_username(), hash_password(), password_needs_rehash(), password_violations(), Path, verify_password(), _weak_passwords() (+13 more)
...
Cohesion: 0.40
Nodes (12): AiBatchRequest, AiGenerationRequest, _native_url(), TestClient, UUID, RecordingDispatcher, _replace_areas(), _session() (+4 more)
...
Cohesion: 0.10
Nodes (26): AuditEventReference, ContractModel, ErrorResponse, FieldError, Page, Pagination, BaseModel, 跨服务使用的公共 Schema 与规范化函数。 (+18 more)
...
Cohesion: 0.19
Nodes (21): create_access_token(), decode_access_token(), hash_refresh_token(), Any, datetime, Access JWT 与 opaque Refresh token 接缝。, _base64url(), _insert_credential() (+13 more)
...
Cohesion: 0.11
Nodes (20): AiTaskCode, JsonValue, Broker, UUID, 仅投递 job_id 的提示词测试分发边界。, RedisJobDispatcher, canonical_json_sha256(), generation_input_sha256() (+12 more)

### Community 99 - "Auth Logic"
Cohesion: 0.16
Nodes (17): ChallengePurpose, StrEnum, AuthResult, _challenge_digest(), _client_challenge(), _decode_base64url(), IdentityError, Any (+9 more)
...
### Community 112 - "AI Generation Presave Tests"
Cohesion: 0.49
...
### Community 140 - "Lesson Plan Calendar"
Cohesion: 0.53
...
- **Why does `IdentityRepository` connect `Identity Repository` to `Identity & Audit`, `Auth Logic`, `Identity Helpers`, `Admin Initialization`, `Auth Throttling`, `AI Generation Service Tests`, `TOTP Encryption`, `Backup Auth Isolation Tests`, `User Management`, `Identity Isolation Tests`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `IdentityError` connect `Auth Logic` to `Identity & Audit`, `Auth Endpoints`, `Audit Events`, `Job Dispatcher`, `Auth & CSRF`, `AI Generation Service`, `AI Job Actors`, `AI Key Rotation`, `WebAuthn Challenges`, `AI Execution Errors`, `User Management`, `Settings Repository`, `Calendar Client`, `Lesson Plan Service`, `Prompt Repository`, `Job Query Service`, `TOTP Encryption`, `Auth Sessions`, `FastAPI Application`, `Backup Maintenance Tests`, `App Health Tests`, `Health Checks`, `Identity Helpers`, `Admin Initialization`, `AI Generation Service Tests`, `Recovery Tests`, `Prompt Test Dispatch`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `ActorFixture` connect `Backup Enrollment Tests` to `Identity & Audit`, `Job Recovery`, `Auth Throttling`, `Invitation Tests`, `TOTP Encryption`, `AI Generation Service Tests`, `Prompt Test API Tests`, `Token Management`, `AI Generation Presave Tests`, `WebAuthn Tests`, `Test Helpers`, `Recovery Tests`, `AI Batch Tests`, `AI Model Profile Tests`, `Backup Maintenance Tests`, `AI Generation Tests`, `AI Preview Lifecycle Tests`?**
