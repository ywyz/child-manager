...
项目已完成 M1 工程骨架与质量基线验收。M2 认证、授权与身份审计仍为 `in_progress`：历史
Codex 实现 Issue [#5](https://github.com/ywyz/child-manager/issues/5) 已关闭并成为新 `dev` 的
实现起点；历史 Trae Issue [#6](https://github.com/ywyz/child-manager/issues/6) 按工作流重置以
`not planned` 归档；M2 Issue [#4](https://github.com/ywyz/child-manager/issues/4) 改为单实现
验收入口。当前 `main` 仍是迁移前 docs-only 基线，只有 `dev` 对齐最新 `docs`、完成完整验证
和 Review 后，才会形成首个可发布 `main` 基线。
