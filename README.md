...
项目已完成 M1 工程骨架与质量基线验收，M2 认证、授权与身份审计现为 `ready`。`main` 仍只
提供共享文档，因此没有可执行的安装、启动或部署入口；`codex`、`trae` 已分别在各自分支
完成 M1。[M2 Issue 草稿与执行记录](docs/development/m2-issue-drafts.md) 已完成只读复核，父 Issue
[#4](https://github.com/ywyz/child-manager/issues/4) 与实现子 Issue
[#5](https://github.com/ywyz/child-manager/issues/5)、[#6](https://github.com/ywyz/child-manager/issues/6)
已经创建；M1→M2 docs-only 内容已发布到 `main`，向实现分支同步共同基线与开始实现仍需
分别授权，后续业务功能按路线图逐阶段推进。
