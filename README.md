...
项目目前处于系统框架与首期业务设计阶段，尚无可用的安装、启动或部署命令。
...
| 分支 | 职责 |
| --- | --- |
| `main` | 当前只维护 README、`docs/`、模板、架构约束和变更记录 |
| `codex` | Codex 的完整独立实现、迁移和测试；生产部署文件后续另行设计 |
| `trae` | Trae 的完整独立实现、迁移和测试；生产部署文件后续另行设计 |

不创建长期 `docs` 分支。`main` 中的文档和模板更新必须使用只包含 `docs/`、`templates/` 或根目录说明文件的独立提交，再分别 cherry-pick 到 `codex` 和 `trae`。

完整的双 Agent 分支边界、Issue 层级、规格冻结、只读交叉评审和交接协议见 [`docs/development/dual-agent-development.md`](docs/development/dual-agent-development.md)。
