...
apps/
├── web/          # NiceGUI 页面与交互
└── api/          # FastAPI、业务服务、数据访问与后台任务入口
packages/
└── contracts/    # Web 与 API 共享的请求/响应契约
```
...
- 自动保存只更新当前内容；AI 生成、手动保存、归档和恢复归档时创建可恢复的历史快照。
...
- 集体活动必须由教师先粘贴纯文本或上传 `.docx` 原始教案，然后由 AI 拆分、补全并新增一个适龄活动环节。
- 首期不支持 PDF 和图片 OCR 教案导入。
- AI 结果必须先进入可编辑表单，未经教师确认不得覆盖已有人工内容。
- 一键生成时各栏目作为独立子任务执行；部分失败不回滚已成功栏目，可只重试失败栏目。

### 异步任务与反馈

AI 生成由 Redis 队列和独立 Worker 异步执行。API 返回任务 ID，Web 通过事件推送或轮询展示状态。
...
业务生成只能读取已发布版本。每次 AI 生成需记录所用提示词版本和模型信息，但不得记录 API 密钥。
...
标准生产部署环境为 Linux + Docker Compose，至少包含：
...
后续子系统应复用统一认证授权、AI 模型档案、提示词管理、异步任务、对象存储和审计能力，避免重复建设基础设施。
