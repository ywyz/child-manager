...
项目采用 `main`、`docs`、`dev` 三条长期分支：`main` 保存稳定版本和发布基线，`docs` 保存
...
M5 [Issue #9](https://github.com/ywyz/child-manager/issues/9) 已按 `completed` 关闭，形成第一条
业务 MVP。M4 [Issue #10](https://github.com/ywyz/child-manager/issues/10) 固定
`docs@c9401c5a189fcc10ee2e15903a186c06b94cea30`，最终 Review SHA
...
T087–T126。T087–T104 已完成；T104 完成提交为
`dev@a75e05576f8dfae98b199ea3411f29bc7f76466a`，自有门禁为 `10 passed`。批量/单栏受理、
任务轮询和预览生命周期薄路由仍按任务边界保留给 T107。当前下一项是 T105 AI 预览采用/
拒绝事务，不得提前把 T107 路由验收并入 T105。
...
首期不开放公众注册。空系统只通过部署控制台的一次性初始化创建待激活首位管理员并发放首个单次邀请；后续账号由管理员创建并发放单次邀请。
...
生产部署实施延后到首期全部功能完成并通过功能验收之后。访问方向已冻结为受限公网，NiceGUI Web/BFF 是唯一公网应用入口，API、Worker、Redis、PostgreSQL 不单独暴露公网；当前仍不创建生产入口、Caddyfile、生产 Compose 拓扑、DNS、证书或公网发布配置。

威胁模型与访问网络方向已经由 ADR-0010 确认。功能完成后据此选择并验证生产边界产品、拓扑、密钥、备份恢复和上线流程；本地开发与自动化测试仍可启动所需的 PostgreSQL、Redis 等最小依赖。

## 分支协作规范

| 分支 | 职责 |
| --- | --- |
| `main` | 稳定版本与发布基线；禁止临时开发和未验收实现 |
| `docs` | PRD、Architecture、ADR、Context、Development Guide、共享 `specs/`、OpenAPI 契约和 `templates/` 的单一事实来源 |
| `dev` | Codex 的唯一实现与集成分支，承载 Python、迁移、UI、测试和依赖锁 |

开发固定遵循：

```text
需求 -> docs 设计与确认 -> GitHub Issue -> dev 实现 -> 测试 -> Review -> main
```

完整规则见 [`docs/development/single-implementation-development.md`](docs/development/single-implementation-development.md)，
本次迁移决策与历史基线见 [`docs/development/workflow-reset-20260721.md`](docs/development/workflow-reset-20260721.md)，
本地依赖、回环端口、数据隔离和中国大陆镜像配置见
[`docs/development/local-development-environments.md`](docs/development/local-development-environments.md)。历史双 Agent
协议仅保留审计用途，不再指导当前开发。

## 质量标准

`dev` 应配置 GitHub Actions，并在每次推送时至少执行：
