...
class SettingsService:
        ...
        try:
            with self._connect() as connection, connection.transaction():
                return SettingsRepository(connection).create_semester(
                    kindergarten_id,
                    name=_normalize_display_name(name),
    ...
    def get_class(self, session: SessionUser, class_id: UUID) -> ClassRecord:
        ...
        kindergarten_id = self._kindergarten_id(session)
        with self._connect() as connection:
            record = SettingsRepository(connection).get_class(kindergarten_id, class_id)
        if record is None:
    ...
    def replace_class_teachers(
        ...
        try:
            with self._connect() as connection, connection.transaction():
                record = SettingsRepository(connection).replace_class_teachers(
                    kindergarten_id,
                    class_id,
                    [
                        TeacherInput(
