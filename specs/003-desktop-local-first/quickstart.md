...
1. 配置一个当前模型和虚构 Key；确认 SQLite/备份/日志中找不到 Key。
2. 单栏生成成功 -> 预览 -> 拒绝，正文与版本不变。
3. 单栏生成成功 -> 修改无关栏 -> 仍可采用；修改目标栏 -> preview stale，拒绝覆盖。
4. 采用有效预览，确认先创建采用前快照再更新正文。
5. 一键四栏使两个成功、一个结构错误重试后成功、一个最终失败；成功预览保留，失败栏可单独
   重试，原正文不变。
6. 发起任务后重复发起，返回单任务提示；取消或关闭应用，迟到结果不写数据库。
7. 重启应用，不恢复未完成任务。
