...
**Branch**: `main`（共享规格；实现分支待用户另行授权） | **Date**: 2026-07-12 | **Spec**: [spec.md](./spec.md)
...
*GATE: 2026-07-13 已关闭 M0-G1～M0-G8，M0 为 `complete`、M1 为 `ready`。这只解除内容门禁，不授权在 `main` 开始业务实现，也不授权创建或切换 `codex`、`trae`；须另行获得 T003 与实现授权。*

| 宪章门禁 | 内部设计检查 | 当前 Pre-M1 实现门禁 | 计划证据 |
| --- | --- | --- | --- |
| I. 事实来源与范围忠实 | PASS | **PASS（M0）** | canonical 文档、历史清理与最终共享基线均已验证 |
| II. 服务边界与单向依赖 | PASS | **PENDING（授权）** | BFF/API/Worker 边界已设计，须经授权启动两个实现分支 |
...
**前置同步与授权处理**: T001 保留为既有 docs-only 同步的历史记录，详情见 [research.md](./research.md) 第 9 节；该旧提交不再作为当前 Pre-M1 通过证据。T002 已在最终候选文档基线上重新完成；T003 只能在 M0-G1～M0-G8 全部关闭并形成最终 docs-only `main` 基线后，经明确授权从同一提交创建 `codex` 和 `trae`。在此之前，任何业务迁移或代码实现均不得开始。
...
**Post-design Constitution Check**: 内部设计、T002 最终 Pre-M1 审查及 M0-G1～M0-G8 均已完成，M0 为 `complete`、M1 为 `ready`；仍须经授权执行 T003，此前不得开始代码任务。
...
2. **US1**: 初始化、认证授权和必要设置。
3. **US2**: 纯手工教案、日期、历史、归档和恢复。
4. **US3**: 模型档案、密钥封装和提示词生命周期/测试。
5. **US4**: 可靠异步任务、一键四栏、独立预览/采用和显式反思。
6. **US5**: 安全导入、集体拆分和新增环节部分成功。
7. **US6**: 固定 Word、独立导出历史和授权下载。
8. **US7**: 审计查询与故障边界降级；性能、安全汇总、可访问性和完整验收由最终 Polish
...
无内部设计复杂度例外；M0 已完成，但不等同于实现授权。仍须经用户明确授权从同一最终 `main` 提交创建 `codex` 和 `trae`，才能分别开始编码。
