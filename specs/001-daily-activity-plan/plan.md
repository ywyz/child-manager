...
HTTPS 单一来源或自动化/本地开发的 `localhost` 安全上下文执行；密码+TOTP 备用登录经同一
Web/BFF 入口，生产拓扑仍延后到 M9
...
33 张首期表、7 个稳定 AI 任务、6 个教案栏目、1 个固定 Word 模板
...
复杂度。33 张表包含通行密钥、challenge、邀请、恢复、身份核验和备用认证所需的 14 张身份
表；M3A 的两张表不恢复旧 `users` 密码列或单因素密码兼容端点，也不增加反思表、环节表或
事务消息系统。
...
- 沿用 19 个业务实体并新增 7 个 WebAuthn/邀请/恢复实体、2 个备用认证实体，连同 5 个既有
  身份实体共 33 个实体；
...
- 身份迁移使用 expand → enroll → contract：先建新表并停用密码端点、撤销旧会话，再由
  ...
  只能重建空列，不恢复旧认证能力。
- M3A 不回滚该历史；在 `0004_settings` 后以
  `0005_password_totp_backup_login` 创建独立密码+TOTP 备用认证表和会话保证字段。
...
- M3A OpenAPI 增加备用状态、绑定 start/verify、双因素 authentication 和 `add_passkey`
  专用 reauthentication；请求秘密标记 `writeOnly`，一次性响应种子标记
  `readOnly + x-sensitive`。
...
- 当前 `main@b7676c27d07adc5eca1f0c397217780367481e9c` 已包含 M1–M3A、M5 与 M4，
  ...
  同时保留后续故事的可执行验收合同，尚未实施的 T105 以后步骤不得声称已经通过。
...
T087–T097 RED 与 T098–T104 已完成；T104
`dev@a75e05576f8dfae98b199ea3411f29bc7f76466a` 自有门禁为 `10 passed`。当前下一项为
T105，其门禁使用不经过 T107 路由的采用/拒绝直接服务测试；批量/单栏受理、任务轮询和
预览生命周期薄路由验收保留给 T107。
...
4. **US1 / M3A**: 独立 `002` 任务在 M3 后完成密码+TOTP 备用登录与认证保证级别。
5. **US2**: 纯手工教案、日期、历史、归档和恢复。
6. **US3**: 模型档案、密钥封装和提示词生命周期/测试。
...
| 密码撞库、TOTP 钓鱼/重放或种子泄露 | 双因素同意图、Argon2id、阻断列表、三层限流、持久 counter、AES-GCM 和保证级别隔离 |
