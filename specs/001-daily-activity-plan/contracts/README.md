...
**Contract revision**: `2.1.0`（保留通行密钥首选流程并增加密码+TOTP 双因素备用登录）
**Base path**: `/api/v1`
...
| POST | `/auth/step-up/verify` | 建立 5 分钟 WebAuthn step-up 证明，不签发新会话 |
| GET/DELETE | `/auth/backup` | 查看备用登录状态；教师经 WebAuthn step-up 后可关闭，管理员不得关闭 |
| POST | `/auth/backup/enrollment` | 经 WebAuthn step-up 或管理员首次受限会话开始密码+TOTP 绑定 |
| POST | `/auth/backup/enrollment/{enrollment_id}/verify` | 同时验证新密码和首个 TOTP 并原子启用 |
| POST | `/auth/backup/authentication` | 密码与 TOTP 同时成功后建立普通业务会话 |
| POST | `/auth/backup/reauthentication` | 仅为新增通行密钥建立 5 分钟专用证明 |
| GET | `/auth/security-events` | 从现有审计事件读取本人最近 20 条备用认证安全警报，不写已读状态 |
| POST | `/auth/refresh` | 轮换 access/refresh；检测重放 |
| POST | `/auth/logout` | 有 Refresh Cookie 时撤销当前 token family；无 Cookie 时仍幂等清除 access/refresh Cookie |
| GET | `/auth/me` | 当前账号、角色、园所展示和当前能力 |
| GET | `/auth/credentials` | 查看本人凭据及名称、创建/最近使用时间，不返回公钥 |
| POST | `/auth/credentials/registration/options` | 在 WebAuthn step-up 或 password_totp 专用再验证后生成新增凭据 options |
...
不存在公众注册、单密码/单 TOTP 登录、短信验证码、远程密码重置或万能恢复入口。密码与
TOTP 只作为二者同时成立的备用登录链；管理员必须绑定，教师可选且持续提示。备用会话可
使用普通业务功能，但其专用再验证只允许新增通行密钥，不能删除旧凭据或修改其他高风险
身份设置。仍有通行密钥时通过 WebAuthn step-up 重置备用因素；全部通行密钥不可用时继续
走离线恢复码与人工核验。CLI 生成 15 分钟单次初始化凭据并在
...
authentication/step-up options 不增加失败计数；只有 verify 失败或授权材料无效才计数。公开
身份端点分别维护可信来源、账号或授权材料摘要、端点全局三层独立阈值，成功不得清空全局失败；
失败计数、challenge 失败结果与脱敏审计不得随 ceremony 业务事务回滚。
