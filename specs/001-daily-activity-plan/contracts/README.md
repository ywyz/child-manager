...
- BFF 必须原样转发方法、规范化路径、query 和 body，以及认证 Cookie、`Origin` 或
  ...
  `Set-Cookie`。登录/刷新各有两条独立 access/refresh Cookie，退出有两条独立过期 Cookie；
...
不设置 `Domain`。Refresh 每次轮换且不延长 token family 的初始 7 天绝对期限；退出、改密、
管理员重置、账号停用和重放检测按规格撤销。

所有状态变更请求，包括登录、刷新和退出，必须：
...
| POST | `/auth/login` | 用户名或手机号 + 密码；通用失败提示 |
| POST | `/auth/refresh` | 轮换 access/refresh/CSRF；检测重放 |
| POST | `/auth/logout` | 撤销当前 token family 并清 Cookie |
| GET | `/auth/me` | 当前账号、角色、园所展示和当前能力 |
| POST | `/auth/change-password` | 校验当前密码，修改后撤销全部 refresh token |
| GET/POST | `/users` | 管理员分页查询/创建账号 |
| GET/PATCH | `/users/{user_id}` | 查看/修改非凭证字段 |
| PUT | `/users/{user_id}/roles` | 设置角色；阻止移除最后管理员 |
| POST | `/users/{user_id}/activate` | 启用账号 |
| POST | `/users/{user_id}/deactivate` | 停用并撤销全部会话 |
| POST | `/users/{user_id}/reset-password` | 管理员重置密码并撤销全部会话 |

不存在公众注册或 HTTP 初始化端点。首位管理员只能用交互式 CLI 初始化。
...
审计响应不得包含完整教案/AI 正文、密码、Cookie、token、API Key 或服务器路径。
...
| 401 | `auth.login_failed` | 通用登录失败，不区分原因 |
...
| 429 | `auth.login_rate_limited` | 来源登录限流 |
