...
不存在公众注册、密码登录/改密/重置端点或万能恢复入口。CLI 只生成 15 分钟单次初始化凭据
并在带外核验后激活账号；通行密钥必须在 HTTPS 或 `localhost` 浏览器 ceremony 中创建，
CLI 不得直接创建或接收凭据。Challenge 使用 32 字节随机值、5 分钟单次有效，并绑定 purpose、
账号/上下文、RP ID 与精确 Origin；注册要求 discoverable credential，常规 ceremony 强制 UV。
