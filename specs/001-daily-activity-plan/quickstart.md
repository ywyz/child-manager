...
M6 [Issue #11](https://github.com/ywyz/child-manager/issues/11) 为 `in_progress` 并覆盖 T087–T126；
US4 T087–T110 已完成；US5 T111–T126 已完成，dev@32d3c102152848f7488da036ddada461b3d8d3ab 的 Quality run 30602225731 在同一 headSha 通过完整检查；
设置页专项 49 passed，US5 专项 56 passed，完整 pytest 666 passed，恶意样本临时残留为 0，Graphify 诊断通过。
下一门禁为固定 `main..dev` Standards/Spec 双轴 Review；`main` 集成仍需单独授权。下列命令
覆盖 M1～M8 验收合同，已完成的用户故事步骤仍需以固定证据复核，不表示 M6 已可集成 main。

## 1. 前提与反目标

需要 Python 3.14、`uv`、PostgreSQL、Redis 和支持 WebAuthn 的浏览器。自动化使用浏览器
虚拟认证器以及固定 AI、工作日和时间替身，不访问真实/付费 AI 或在线节假日服务，也不
要求实体安全密钥。
...
责任人信息；在一个事务中创建园所、角色种子、`pending_registration` 管理员和 15 分钟
单次初始化记录。CLI 只展示一次至少 128 bit 随机凭据，数据库只保存摘要；入口和凭据分开
展示，命令行参数、环境变量、日志和审计均不得携带明文。CLI 不创建通行密钥，也不接收
浏览器 credential JSON。

浏览器消费初始化凭据后生成注册 options 并完成首个通行密钥登记，账号进入
`pending_verification`，且不得建立会话。两名预登记核验人完成带外核验后，`activate` 才能
...
API 仅信任显式配置的回环 BFF，伪造地址不得绕过或嫁祸公开身份端点来源限流。
认证完成和刷新响应必须各保留两条独立 access/refresh `Set-Cookie`；退出响应必须保留两条独立
...
curl --fail "http://localhost:${CHILD_MANAGER_WEB_PORT}/"
# 以下 CSRF 业务端点仅在 T033 完成后检查：
curl --fail --cookie-jar "$CHILD_MANAGER_COOKIE_JAR" \
  "http://localhost:${CHILD_MANAGER_WEB_PORT}/api/v1/auth/csrf"
...
不就绪。浏览器通过 `http://localhost:${CHILD_MANAGER_WEB_PORT}` 访问 WebAuthn 允许的本地
安全上下文并进入中文登录页，CSRF 响应及 Cookie
...
### 6.1 初始化、通行密钥、恢复与会话验收矩阵

打开 `http://localhost:${CHILD_MANAGER_WEB_PORT}/`，由同源 BFF 的 `/api/v1/auth/csrf`
取得签名 CSRF Cookie；WebAuthn 只在 HTTPS 或 `localhost` 安全上下文运行，浏览器不得请求
`http://127.0.0.1:${CHILD_MANAGER_API_PORT}/api/v1/*`。缺失/伪造 CSRF 或错误 Origin 的状态
变更必须得到 `403 auth.csrf_invalid`。

| 流程 | 正向证据 | 必测拒绝路径 |
| --- | --- | --- |
| 首位管理员 | `init-admin start` → 浏览器注册 → 双人核验 → CLI 激活，恢复码只展示一次且不建会话 | CLI 直接创建凭据、秘密入 URL/日志、过期/重放、缺少任一核验人 |
| 教师邀请 | 管理员签发最多 24 小时邀请，教师完成首个凭据后经管理员核验激活 | 过期、撤销、已消费、并发消费、跨账号/园所使用，登记后直接建会话 |
| 注册 ceremony | options 可直接传给浏览器 WebAuthn API，verify 保存公开凭据与计数器 | challenge 过期/重放/跨 purpose、type、RP ID、Origin、UV、签名或账号绑定错误 |
...
uv run pytest tests/contract/test_auth_contract.py tests/contract/test_users_contract.py
