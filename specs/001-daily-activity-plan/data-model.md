...
**Scope**: 33 张首期表；新增 WebAuthn、邀请、恢复及两张密码+TOTP 备用认证表，不增加
反思表、集体活动环节表或未来子系统表
...
关键字段：`username`、`username_normalized`、可空 `phone_e164`、`display_name`、随机 32 字节
`webauthn_user_handle`、`status`、`backup_auth_version`、`activated_at`、`last_login_at`、
`created_by`、`updated_by`。

- 唯一 `(kindergarten_id, username_normalized)`。
- `phone_e164 IS NOT NULL` 时唯一 `(kindergarten_id, phone_e164)`。
- `webauthn_user_handle` 全局唯一且不含用户名、手机号或其他 PII。
- 停用后继续占用用户名和手机号；不级联删除历史引用。
- `status` 只允许 `pending_registration/pending_verification/active/suspended`；注册首个凭据后
  仍需带外核验才 active。表中不含密码或 TOTP 字段；M2 删除的旧密码列不恢复。

### 3.3 `webauthn_credentials`

保存同园用户、唯一 Credential ID、COSE 公钥、签名计数、transport、AAGUID、BE/BS、标签、
`created_via`、最近使用和撤销元数据。只保存 W3C 验证所需最小字段，不保存私钥、PIN 或
生物特征。自助撤销不得移除本人最后一个有效凭据；管理员撤销教师最后凭据时同步撤销会话并
转为 `pending_registration`。

### 3.4 `webauthn_challenges`

保存至少 32 随机字节 challenge 的哈希、purpose、可空用户、授权上下文、冻结 RP ID/Origin、
最长 5 分钟到期、消费时间和失败次数。purpose 为 bootstrap/invitation/self-add/recovery
registration、authentication 或 step-up；不同 purpose 不可互换，完成 ceremony 与相应状态
变更在同一事务消费。

### 3.4A `backup_auth_credentials`

每账号最多一行当前材料：Argon2id `password_hash`、密码变更时间、AES-256-GCM TOTP
密文/12 字节 nonce/`key_id`/envelope version、固定 `SHA1/6/30` 参数、原子
`last_accepted_counter`、`enabled/revoked` 状态和时间。唯一
`(kindergarten_id, user_id)`；`enabled` 要求摘要与完整信封一起存在，`revoked` 必须清除
它们。AAD 绑定园所、用户、凭据和版本。

### 3.4B `backup_auth_enrollments`

保存同园用户、发起 refresh token、待确认 TOTP 密文信封、10 分钟到期、消费和作废状态。
部分唯一约束只允许一个未终结绑定；新绑定原子作废旧绑定，token 轮换使绑定失效。密码不
进入本表。enrollment AAD 在成功时以新 nonce 重加密并改绑 credential ID；验证成功在同一
事务消费绑定、启用新材料、增加 `backup_auth_version` 并撤销旧备用会话/证明。完整字段见
[`specs/002-password-totp-backup-login/data-model.md`](../002-password-totp-backup-login/data-model.md)。
...
保存目标账号、签发管理员、至少 128 位随机令牌摘要、最长 24 小时到期、消费/撤销及注册凭据。
重新签发先撤销旧邀请；凭据保存与邀请消费原子完成，但账号仅进入 `pending_verification`。

### 3.7 `recovery_codes`

每个账号至多一个有效离线恢复码；只保存至少 128 位随机码摘要、签发/消费/撤销和替换关系。
激活后的首次成功通行密钥认证、主动轮换和恢复完成只向用户展示一次新原始码并原子作废旧码；
管理员激活响应和 CLI 不接触原始码。

### 3.8 `account_recovery_requests`
...
关键字段：`token_family_id`、唯一 `token_hash`、`issued_at`、`expires_at`、`last_used_at`、
`authentication_method`、`webauthn_verified_at`、`backup_verified_at`、
`backup_reauthenticated_at`、`backup_auth_version`、`revoked_at`、`revoke_reason`、
`replaced_by_id`、脱敏 `client_label`。

- Refresh Token 是随机 256 位不透明值，只保存强哈希；`expires_at` 保存族首次签发后 7 天的
  固定绝对到期时间，同一 family 的后续轮换记录沿用该值，不得延长。
- 每次刷新撤销旧 token 并指向新 token；退出、会话撤销、停用、最后凭据撤销和恢复完成
  撤销相应或全部 token。
...
- Access Token 不建表：15 分钟 HS256 JWT，签名密钥在数据库外，claims 含最小身份与
  `sid=token_family_id`；每次请求仍查询账号、角色、班级和 family 撤销状态。
- `password_totp/restricted_enrollment` family 的备用因素版本必须与用户一致；WebAuthn
  family 不因备用因素版本变化失效。备用重新验证时间只允许 `add_passkey`，WebAuthn
  重新验证时间保护其他高风险身份操作。
...
`occurred_at`。只插入且至少保留一年；不得保存密码、密码摘要、TOTP、种子、二维码、token、
Cookie、API Key、完整教案或完整 AI 输入输出。
备用认证事件使用 `resource_type=user/resource_id=被通知用户`，本人安全事件端点仅按当前
园所和本人投影最近 20 条白名单事件，不新增通知表或已读状态。

## 9. 关键事务与状态不变量

| 用例 | 同一事务内必须完成 |
| --- | --- |
| 首次初始化 | 园所、角色种子、待注册管理员、角色分配、初始化凭据摘要；任一步失败全部回滚 |
| WebAuthn 注册/认证 | challenge 行锁与消费、凭据保存或 session/family 签发、审计 |
| 备用绑定/登录 | enrollment 消费、新材料启用或密码+TOTP 同意图验证、TOTP counter 原子推进、备用因素版本/会话/审计 |
| 邀请注册/激活 | 保存首个凭据、消费邀请、推进 pending_verification；核验后激活，首次成功认证再向用户签发首个恢复码 |
| 凭据/会话撤销 | 最后凭据/最后管理员保护、凭据撤销、相应 Refresh family 撤销、账号状态 |
| 恢复完成 | 保存新凭据、撤销旧密码/TOTP/通行密钥/会话/邀请/恢复码、签发新恢复码、完成请求、审计 |
...
1. 园所、用户状态、WebAuthn、初始化、邀请、恢复、角色、refresh token、审计；启用 PostgreSQL `btree_gist`。
2. 年龄段、班级、教师关系、学期、区域。
3. 密码+TOTP 备用认证表与会话保证字段。
4. 教案、作者、快照和工作日缓存。
5. 通用后台任务、幂等索引、租约与恢复扫描所需索引。
6. AI 模型、能力、提示词定义/版本/异步测试，以及引用前述实体的 AI 预览。
7. 集体活动来源、Word 导出和最终组合外键/索引。
...
身份迁移必须兼容当前 `dev` 的密码型 M2 数据：expand 新增身份结构、立即删除密码路由并撤销
旧 session；enroll 只在零 WebAuthn active 管理员时允许 CLI 为一个既有管理员签发短时
`migration_admin` 登记凭据；contract 确认登记门禁后删除 `password_hash`、
`password_changed_at` 和 `is_active`。contract downgrade 只重建空列，不恢复旧单因素密码
认证；数据回退使用停机备份。M3A 在独立表增加必须与 TOTP 同时验证的新摘要，
`0005_password_totp_backup_login` 继承 `0004_settings`；下游尚未实施迁移顺延为
`0006`–`0010`。其余领域直接采用最终约束：周次双空、
`unavailable`、模型默认并发 2、栏目基线哈希与 `PlanContentV1`。

## 11. 必测数据行为

- 同园同班同日唯一，归档后仍不能重复创建；跨园组合外键和 Repository 负向测试。
- challenge 的 purpose/过期/消费、Credential ID、邀请、恢复码和恢复请求的唯一与并发消费。
- 首位管理员 CLI 不创建通行密钥；邀请注册不激活账号；缺少人工核验不能登录。
- 密码策略/Argon2id、TOTP 时间窗口/原子 counter、种子 AEAD、备用账号枚举/限流、
  管理员门禁和 `add_passkey` 专用证明。
- 恢复完成撤销包含密码/TOTP 在内的全部旧认证材料但不建立会话，新恢复码只展示一次。
- expand/enroll/contract 覆盖含密码账号与 Refresh 数据的真实 PostgreSQL 边界，contract 后
  `users` 物理 Schema 不含旧密码字段且旧单因素路径不存在。
