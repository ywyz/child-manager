...
**Scope**: 31 张首期表；新增 WebAuthn、邀请与恢复身份表，不增加反思表、集体活动环节表或未来子系统表
...
`webauthn_user_handle`、`status`、`activated_at`、`last_login_at`、`created_by`、`updated_by`。
...
`last_reauthenticated_at`、`revoked_at`、`revoke_reason`、`replaced_by_id`、脱敏 `client_label`。
...
| 恢复完成 | 保存新凭据、撤销旧凭据/会话/邀请/恢复码、签发新恢复码、完成请求、审计 |
...
`password_changed_at` 和 `is_active`。contract downgrade 只重建空列，不恢复密码认证；数据
回退使用停机备份。其余当前 `main` 尚无业务迁移的领域直接采用最终约束：周次双空、
