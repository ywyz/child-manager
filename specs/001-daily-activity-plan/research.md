...
**Date**: 2026-07-12
**Updated**: 2026-07-13
**Status**: 研究决策已收敛；实现仍受 Pre-M1 canonical 文档同步授权门禁阻断
...
以下行为已经由用户明确确认，不再是产品待决项；但当前共享文档仍有旧表述。任何业务
实现或初始 Alembic 迁移开始前，必须先经用户授权在 `main` 单独同步，再 cherry-pick 到
实现分支：

| 共享文档漂移 | 本 feature 已冻结规则 |
| --- | --- |
| `AGENTS.md`、`README.md` 的“AI 生成建快照” | 只有教师采用 AI 结果建快照 |
| README/PRD 的“一键含反思” | 一键固定四栏；反思五栏完整后显式触发 |
| PRD 的两类区域均须完整 | 班级可不完整；仅目标 AI 栏目按类别校验 |
| PRD 的 Redis 故障即任务失败 | DB 保存后即受理并处于 `pending_dispatch` |
| 数据模型/Schema 的周次非空 | 学期外两个周次字段同时为空 |
| 工作日 `source_code` 无 `unavailable` | 两来源不可用保存 `unknown/unavailable` |
| 全局版本变化使预览失效 | 目标栏目基线 + 实际输入哈希决定有效性；写入仍乐观锁 |
| 作者必须始终是当前班级教师 | 新增作者须当前关联；解绑保留旧署名并立即撤权 |
| 集体活动两项结果未分离 | 拆分和新增环节分别预览、采用和建快照 |
| 模型档案默认并发为 1 | 默认 2，可配置 |
| 提示词仅泛称“变量占位符” | 只允许 `\{\{[ \t]*([a-z][a-z0-9_]*)[ \t]*\}\}` 白名单纯替换和稳定 JSON |

该同步任务不得扩大到生产部署、照片/对象存储或后续子系统。
