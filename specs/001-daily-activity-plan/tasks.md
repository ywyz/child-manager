...
- [ ] T001 在用户授权后同步 `AGENTS.md`、`README.md`、`CONTEXT.md`、`docs/PRD/lesson-management.md`、`docs/design/system-architecture.md`、`docs/design/data-model.md`、`docs/design/database-schema.md`、`docs/ROADMAP.md`，统一仅采用建快照、一键四栏、显式反思、区域按栏、`pending_dispatch`、学期外空周次、`unavailable`、栏目/输入哈希、解绑保留署名、模型默认并发 2、导出仅以前五栏判缺且空反思保留三行，以及 `\{\{[ \t]*([a-z][a-z0-9_]*)[ \t]*\}\}` 白名单纯替换；验证：第一条 `rg -n "AI 生成和重新生成必须创建关键版本快照|一键生成只生成集体活动以外的栏目|一键生成时包含反思栏目|每类区域至少 1 项|Redis 或 Worker 不可用.*任务创建失败" AGENTS.md README.md CONTEXT.md docs` 对旧规则无未解释命中；第二条 `rg -n "\[a-z\].*\[a-z0-9_\]" README.md CONTEXT.md docs` 至少命中精确提示词词法；第三条 `rg -n "空反思.*不触发|反思为空.*不触发" README.md CONTEXT.md docs` 至少命中导出规则；`git diff --check` 通过
- [ ] T002 对 `AGENTS.md`、`README.md`、`CONTEXT.md`、`docs/`、`specs/001-daily-activity-plan/` 执行 Pre-M1 一致性审查并准备单一目的 docs-only 变更；验证：`/speckit-analyze` 无 CRITICAL/HIGH 未解释项，`git diff --name-only` 不含 `apps/`、`packages/`、迁移或测试；只有用户另行授权提交后才可提交，否则阻断 T003
...
- [ ] T039 [P] [US1] 在 `apps/web/pages/auth.py`、`apps/web/components/navigation.py`、`apps/web/api_client.py` 实现中文登录页、Cookie/CSRF 交换、按角色导航和可信来源转发；raw multi-header 必须保留登录/刷新/退出的两条 Cookie，伪造来源头被丢弃重建，Web 不缓存授权；验证：`uv run pytest tests/web/test_us1_setup_smoke.py -k 'login or logout or refresh or navigation or spoofed_client_ip' tests/web/test_bff_proxy.py tests/unit/identity/test_client_ip.py` 通过，网络记录无 8000 请求且无 Cookie 折叠
