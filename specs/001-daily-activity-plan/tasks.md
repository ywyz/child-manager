...
固定引用不可移动 docs SHA；T087–T104 已完成，当前下一项为 T105，并保持 T087–T110
先于 T111–T126。
...
- [ ] T105 [US4] 在 `packages/backend/lesson_plans/ai_adoption.py` 实现重新鉴权、双哈希、版本 CAS、正文合并、单快照、任务/结果状态和审计的采用事务；先新增 `tests/unit/lesson_plans/test_ai_adoption_service.py` 直接验证采用、拒绝与幂等事务；验证该直接服务测试通过，预览过期维护仍属于 T104，`tests/api/test_ai_preview_adoption.py` 与 `tests/api/test_ai_preview_lifecycle.py` 的薄路由生命周期验收保留给 T107
