...
class TeacherplanTemplateError(ValueError):
    error_code = "export.template_changed"
...
class TeacherplanRenderer:
    def __init__(self, template_path: Path, *, expected_sha256: str) -> None:
        ...
        if sha256(self._template_bytes).hexdigest() != expected_sha256:
    ...
    def render_day(self, snapshot: DailyPlanExportSnapshot) -> bytes:
        ...
        self._render_header(document, snapshot)
        self._render_table(document, snapshot)
        output = BytesIO()
    ...
    def _render_header(
        ...
        semester_range = self._semester_month_range(
            snapshot.semester_start_date,
            snapshot.semester_end_date,
        )
        self._replace_paragraph(
            document.paragraphs[0],
            [(f"{snapshot.kindergarten_name}一日活动计划（{semester_range}）", False)],
            font_name="楷体",
            font_size=16,
        )
        self._replace_paragraph(
            document.paragraphs[1],
            [(f"{snapshot.class_name} {snapshot.author_name}".strip(), False)],
            font_name="楷体",
            font_size=16,
        )
    ...
    def _render_table(
        ...
        table = document.tables[0]
        content = snapshot.content.model_dump(mode="python")
        self._set_cell(table.cell(0, 0), snapshot.teaching_week_text)
        self._set_cell(table.cell(1, 0), snapshot.activity_date_text)
    ...
    def _render_area(self, table: Table, start_row: int, area: Mapping[str, object]) -> None:
        ...
        self._set_cell(
            table.cell(start_row + 1, 1),
            self._guidance_text(area),
        )
        self._set_cell(
    ...
    @staticmethod
    def _guidance_text(value: Mapping[str, object]) -> str:
        return "\n".join(
            (
    ...
    def _semester_month_range(start: object, end: object) -> str:
        def month(value: object) -> str:
            ...
            return f"{int(parts[0])}.{int(parts[1])}" if len(parts) >= 2 else str(value)
    ...
    def _set_cell(
        ...
        *,
        ...
        for index, paragraph in enumerate(paragraphs):
            assigned = (
                self._join_lines(lines[index:])
                if index == len(paragraphs) - 1
                else (lines[index] if index < len(lines) else [])
            )
            self._replace_paragraph(
                paragraph,
                assigned or [("", False)],
