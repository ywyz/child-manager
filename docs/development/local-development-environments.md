...
本文定义 `dev` 单实现分支的本地开发与自动化测试环境，隔离 Git 工作区、宿主机端口、Compose 资源、数据库、缓存和运行时文件，并说明中国大陆网络环境下 Python 包与容器镜像的可选加速方式。
...
- [`single-implementation-development.md`](single-implementation-development.md)：单实现分支、Issue、文档同步、Review 和发布协议。
...
推荐为 `main`、`docs`、`dev` 使用不同工作树，避免在同一目录反复切换长期分支或覆盖未提交文件：

```text
<workspace>/child-manager       main：稳定发布基线
<workspace>/child-manager-docs  docs：文档与契约
<workspace>/child-manager-dev   dev：Codex 实现与测试
```

在分支和工作树操作已获授权、目标目录不存在且分支未被其他工作树占用时，可由维护者执行 `git worktree add`。只有 `dev` 工作树创建 `.venv`、运行应用和执行测试；`docs` 工作树不得生成实现依赖或运行时文件。各工作树可以共享 Git 对象库与 uv 全局下载缓存，但不得共享未提交文件、数据库、导出目录、临时目录或开发密钥文件。

## 4. 固定本地档位

`dev` 使用以下宿主机端口。容器内部仍使用 PostgreSQL `5432` 和 Redis `6379`；宿主机端口只绑定 `127.0.0.1`，不得绑定 `0.0.0.0`、`::` 或局域网地址。

| 项目 | Dev 档位 |
| --- | --- |
| `COMPOSE_PROJECT_NAME` | `child_manager_dev` |
| NiceGUI Web | `18080` |
| FastAPI API | `18000` |
| PostgreSQL 宿主机端口 | `15432` |
| Redis 宿主机端口 | `16379` |
| 开发数据库名 | `child_manager_dev` |
| 测试数据库名 | `child_manager_dev_test` |
...
- Compose 项目名固定为 `child_manager_dev`，避免与机器上的其他项目冲突。
- PostgreSQL、Redis 的宿主机端口只绑定回环地址。
- 数据库名、测试数据库名和测试 Worker 标识包含 Dev 档位；并行测试时增加进程或 worker 后缀。
- 导出、上传解析、Cookie jar、日志和临时文件使用 `CHILD_MANAGER_RUNTIME_ROOT` 下的专用目录；测试结束只清理自己创建的文件。
- `CHILD_MANAGER_RUNTIME_ROOT` 是运行文件的唯一根目录；Quickstart 在其下创建 `exports/`、`logs/` 与 `tmp/`，Cookie jar 也写入该目录。实现不得静默回退到共享固定目录。
- 开发密钥只在当前 shell 或仓库外、权限受控的文件中生成，不写 `.env`、Compose 文件、日志或 Git。
- 停止环境时显式带上 `COMPOSE_PROJECT_NAME`，不得使用会影响其他项目的全局容器或卷清理命令。
...
- `dev` 维护、提交并验证唯一的 `uv.lock`；`docs` 不修改依赖锁。
...
终端先加载 Dev 档位变量，再按 [`quickstart.md`](../../specs/001-daily-activity-plan/quickstart.md) 启动依赖、API、Worker 和 Web。浏览器只访问 NiceGUI Web；不得直连 API。

验收至少确认：

1. 当前分支为 `dev`，工作区没有被 `main` 或 `docs` 的未提交文件污染。
2. PostgreSQL、Redis 和应用端口只绑定 `127.0.0.1`。
3. 浏览器只访问 NiceGUI Web，BFF 只访问本档位 API。
4. 数据库、Redis、导出、日志、Cookie jar 和临时文件都属于 Dev 档位。
5. 停止或破坏 PostgreSQL/Redis 时，应用就绪状态和中文错误反馈符合契约。
6. 镜像加速入口关闭后，取消覆盖即可回到官方镜像，不需要修改受版本控制文件。

未实际运行的环境、浏览器或依赖检查不得写成通过。
