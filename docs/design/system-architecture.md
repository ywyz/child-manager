...
本文定义 Child Manager 的目标系统架构、服务边界、依赖方向、后台任务可靠性和安全结果。Codex 的 `dev` 实现可以自主选择类名和内部组织，但必须遵守本文的外部边界、数据所有权和运行时约束。根据 [ADR-0010](../ADR/ADR-0010-restricted-public-entry-passkey-authentication-and-recovery.md)，受限公网入口、NiceGUI Web/BFF 唯一公网应用入口、WebAuthn 通行密钥和紧急恢复边界已经冻结；根据 ADR-0009，反向代理产品、部署拓扑、密钥托管实现和备份运维仍延后到全部功能完成后复审。
...
- `identity`：账号、角色、WebAuthn 凭据、邀请、恢复、会话和班级访问权限。
...
| --- | --- | --- | --- |
| `identity` | API 认证与管理用例 | 完成 WebAuthn ceremony，从会话建立可信身份上下文，并回答园所、角色和班级能力 | RP/Origin/challenge 校验、凭据生命周期、邀请与恢复、令牌轮换/撤销、最后管理员不变量 |
...
- 每个账号允许绑定多个通行密钥。新增或删除凭据及高风险管理员操作要求新近 WebAuthn 用户验证。
...
- 普通用户由有效管理员通过 Web/API 核验；最后管理员的 Web/API 审批必须返回
  ...
  撤销旧凭据、全部会话、待处理邀请和旧恢复码。
- 微信小程序 MFA 不进入首期认证关键路径，也不得成为密码、OTP 或恢复码单独登录的替代入口。
...
功能开发期间只定义密钥来源接口和测试适配器。生产适配器、文件权限、轮换操作和密钥带外恢复必须等 ADR-0009 复审后再实现；账号通行密钥恢复另按 ADR-0010 执行，不与主加密密钥恢复混用。
...
- 认证安全测试覆盖 WebAuthn challenge、Origin/RP ID、UV、签名、重放、账号枚举、多凭据、邀请泄露、双条件恢复和最后管理员恢复。
...
| 常规登录 | WebAuthn 通行密钥，要求 RP/Origin 绑定和用户验证 | 提供钓鱼抗性，不保留密码或 OTP 弱兜底 |
...
3. 实现 WebAuthn 登录、邀请与恢复、Cookie 会话、角色和教师—班级权限。
...
10. 全部功能完成后，依据已确认威胁模型和 ADR-0010 启动独立生产部署实现复审。
