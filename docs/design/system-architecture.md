...
6. Web/BFF 服务端调用 API 时转发当前请求的认证 Cookie；API 负责令牌轮换、撤销、账号停用和权限判断。
...
- 空系统只允许通过本机/部署控制台的一次性初始化建立园所和待激活首位管理员，并发放首个单次邀请；不得提供公网初始化端点或默认管理员。
...
- 首次激活后签发只展示一次、服务端只保存摘要的离线恢复码。紧急恢复必须同时满足有效恢复码和人工核验，恢复码本身不建立会话。
