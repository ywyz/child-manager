...
本文定义 Child Manager 的目标系统架构、服务边界、依赖方向、运行拓扑、后台任务可靠性、安全边界和部署约束。Codex 与 Trae 的实现可以在类名和内部组织上存在差异，但必须遵守本文的外部边界、数据所有权和运行时约束。

本文描述的是目标设计，不表示当前 `main` 已经存在业务实现。当前实现状态以 [`CONTEXT.md`](../../CONTEXT.md) 为准；一日活动计划的用户行为和验收标准以 [`docs/PRD/lesson-management.md`](../PRD/lesson-management.md) 为准。
...
| 运行单元 | 核心职责 | 禁止事项 |
| --- | --- | --- |
| Caddy | HTTPS、统一域名、路由、安全响应头 | 不承载业务规则或权限判断 |
...
- PostgreSQL、Redis、Worker 与容器管理端口不对公网开放。
...
改变状态的请求应同时采用 `SameSite` Cookie、来源校验和 CSRF 防护。Caddy 与应用仅信任明确配置的代理头，避免伪造客户端地址或协议。
...
- 非敏感部署配置使用环境变量，例如数据库地址、Redis 地址、公开域名和日志级别。
- 数据库口令、令牌签名密钥、AI Key 解密主密钥等使用 Docker Secrets 兼容的只读文件挂载。
- API 与 Worker读取同一主密钥文件；主密钥不得进入代码仓库、数据库、镜像、普通环境变量或日志。
- AI API Key 使用经认证加密后保存于 PostgreSQL，页面只显示脱敏值。
- 密钥读取封装为适配器，未来可替换为云密钥管理服务而不改变业务模块。

### 13.2 安全边界

- 仅 Caddy 暴露公网端口，容器间使用私有网络。
...
- `migrate` 成功后才启动新版本 API 与 Worker。
- PostgreSQL 数据、Redis 必要持久化数据和导出文件使用独立持久化卷。
- 模板以只读方式挂载；Secrets 仅挂载到需要它的服务。
- Web、API 和 Worker 使用同一代码版本与契约版本。
- 生产发布由管理员手动触发，首期不自动发布到生产。

### 15.2 备份与恢复

- PostgreSQL 与导出副本每日备份，至少保留 30 天。
- 备份加密后保存于应用服务器之外。
- 数据库迁移和版本升级前先创建可验证备份。
- 至少定期在隔离环境执行恢复演练并记录结果。
- 首期目标为 RPO 24 小时、RTO 4 小时。
- 审计记录至少保留一年；导出与业务数据按后续正式保留策略清理。

Redis 不承担唯一业务数据，灾难恢复时允许依据 PostgreSQL 中未完成任务重建队列。
...
- 部署冒烟测试覆盖 Caddy HTTPS 路由、Cookie 属性、迁移、健康检查、导出下载和备份任务。
- 恢复演练证明可从数据库与导出备份满足既定 RPO/RTO。
...
- 数据表、索引、外键、任务租约与幂等键的精确定义。
- API 端点、错误码、分页和契约 Schema。
- JWT/会话时长、刷新令牌撤销表和 CSRF 令牌细节。
- Caddyfile、Compose 资源限制和生产主机目录规划。
- AI 加密算法、密文封装版本和主密钥轮换流程。
- 备份工具、异地存储介质、恢复演练频率和告警渠道。
- 各业务模块的详细设计与页面信息架构。
