...
状态：功能架构及访问/认证安全边界已确认，生产部署实现已延后
...
本文定义 Child Manager 的目标系统架构、服务边界、依赖方向、后台任务可靠性和安全结果。
Codex 的 `dev` 实现可以自主选择类名和内部组织，但必须遵守本文的外部边界、数据所有权和
运行时约束。根据
[ADR-0010](../ADR/ADR-0010-restricted-public-entry-passkey-authentication-and-recovery.md)，
受限公网入口、NiceGUI Web/BFF 唯一公网应用入口、WebAuthn 和紧急恢复边界已经冻结；
[ADR-0011](../ADR/ADR-0011-password-totp-backup-login.md) 增加密码与 TOTP 双因素备用登录，
并保留 WebAuthn 保护的高风险身份操作边界。根据 ADR-0009，反向代理产品、部署拓扑、密钥
托管实现和备份运维仍延后到全部功能完成后复审。
...
2. NiceGUI Web、FastAPI API 与 Worker 职责清晰，可独立运行和测试；只有 Web/BFF 可以成为公网应用入口，生产拓扑实现延后决定。
...
```mermaid
flowchart LR
    U[教师 / 管理员] -->|受限公网 HTTPS| E[公网边界]
    E --> W[NiceGUI Web / BFF<br/>唯一公网应用入口]
    W -->|私有网络转发 Cookie 与 API 请求| A[FastAPI API]
...
| 运行单元 | 核心职责 | 禁止事项 |
| --- | --- | --- |
| 受限公网边界（产品待选） | 单一 HTTPS Origin、默认拒绝、安全响应头、限流、请求上限和可信代理链 | 不把 FastAPI API 或任何数据/运维端口直接转发到公网 |
| NiceGUI Web/BFF | 唯一公网应用入口；页面、交互、表单状态、用户反馈和私有 API 转发 | 不连接数据库，不导入 ORM/Repository，不决定权限，不允许浏览器绕过 BFF 直连 API |
| FastAPI API | 私有运行单元；认证授权、输入校验、事务、业务编排、任务创建、文件鉴权下载 | 不暴露公网，不执行耗时 AI 调用，不把关键规则留给 Web |
...
- `identity`：账号、角色、WebAuthn 凭据、密码+TOTP 备用因素、认证保证级别、邀请、恢复、
  会话和班级访问权限。
...
| --- | --- | --- | --- |
| `identity` | API 认证与管理用例 | 完成 WebAuthn 或密码+TOTP 备用认证，从会话建立可信身份上下文，并回答园所、角色、班级和身份操作保证级别 | RP/Origin/challenge、Argon2id、TOTP 重放/种子加密、凭据生命周期、邀请与恢复、令牌轮换/撤销、最后管理员不变量 |
...
- `/`、页面资源、认证 ceremony、受控下载和必要业务请求均先到 NiceGUI Web/BFF。
- Web/BFF 通过私有网络调用 FastAPI API；浏览器不得通过公网地址、端口或网关旁路直达 API。
- FastAPI API、PostgreSQL、Redis、Worker 与管理端口不得直接暴露给用户访问网络。

生产环境使用哪种网关、反向代理、证书和拓扑仍延后决定，但这些实现不得改变 Web/BFF 唯一公网应用入口。功能开发阶段可在受控本地环境中提供等价同源入口以验证 Cookie、CSRF、WebAuthn 和路由行为。
...
1. Web/BFF 发起 WebAuthn 注册或认证 ceremony，API 创建高熵、短时、单次且绑定预期 ceremony 的 challenge。
2. 浏览器通过通行密钥完成用户验证，Web/BFF 将 assertion 转发给 API。
3. API 验证 type、challenge、精确 Origin、RP ID hash、用户在场/验证标志、签名和凭据归属。
4. API 验证成功后签发短期 access token 与可撤销 refresh token，Web/BFF 将 Cookie 属性原样交付浏览器。
5. Cookie 默认必须启用 `Secure`、`HttpOnly` 和合适的 `SameSite`，令牌不得进入 JavaScript、`localStorage`、日志或 NiceGUI 持久化存储。
6. Access token 携带会话 `sid`；Web/BFF 服务端调用 API 时转发当前请求的认证 Cookie，API
   每次请求实时检查会话/账号状态，并负责 Refresh family 轮换、重放撤销、账号停用和权限判断。

密码与 TOTP 备用链路由同一 `identity` 模块处理，不创建第二套会话系统：

1. Web/BFF 在一个请求意图中提交规范化账号标识、密码和 TOTP；API 对未知账号执行等价
   Argon2id/虚拟 TOTP 路径，并使用通用公开失败。
2. TOTP 验证只接受当前时间步及相邻一个时间步，并通过 PostgreSQL 原子推进
   `last_accepted_counter`；相同或更早计数器不能再次成功。
3. 两项同时正确后签发相同 Cookie 形态的可撤销会话，但持久化
   `authentication_method=password_totp`、备用因素版本和备用验证时间。
4. 普通业务授权仍由角色、园所和班级决定；身份操作另检查保证级别。最近五分钟密码+TOTP
   再验证只产生 `add_passkey` 专用证明，不能被其他端点复用。
5. 新通行密钥注册成功消费专用证明，但不自动把原会话提升为 WebAuthn 会话。
6. 本人安全事件列表从 `audit_events` 按当前园所和本人投影最近 20 条备用认证事件；Web
   在登录后读取并展示，不新增通知表、已读状态或外部消息投递服务。

改变状态的请求应同时采用 `SameSite` Cookie、来源校验和 CSRF 防护。最终访问入口与应用仅信任明确配置的代理头，避免伪造客户端地址或协议。

为了在 ADR-0009 延后生产 HTTPS 实现的功能阶段完成真实浏览器登录冒烟，允许一个严格受限的开发例外：
...
- 空系统只允许由本机/部署控制台 CLI 在事务中建立园所和待激活首位管理员，并生成 15 分钟
  单次初始化凭据；凭据只展示一次、数据库只存摘要，入口与凭据分开交付。CLI 不直接创建
  通行密钥，也不接收浏览器 credential JSON；管理员必须在 HTTPS/`localhost` 浏览器完成
  首个通行密钥登记，再由预登记园所负责人和独立运维/安全责任人带外核验，最后由 CLI 激活。
  不得提供公网初始化入口、秘密 URL 或默认管理员；登记与恢复均不直接建立会话。
- 常规账号由管理员先创建，再发放最多有效 24 小时、可撤销、单次使用且只保存摘要的邀请；邀请只注册首个通行密钥，管理员带外核验并确认激活后才能访问业务数据。
- 每个账号允许绑定多个通行密钥。密码+TOTP 备用会话经五分钟专用再验证只可新增通行密钥；
  删除/命名凭据、备用因素维护及高风险管理员操作仍要求新近 WebAuthn 用户验证。
- 管理员必须完整配置备用因素，教师可选。密码与 TOTP 只在首次 TOTP 验证成功后原子启用；
  因素替换/关闭增加备用因素版本并撤销旧备用会话和专用证明，不撤销仍有效的 WebAuthn
  会话。管理员首次绑定成功后，当前受限会话转换或重新签发为普通 WebAuthn 会话。
- 用户可以查看、命名、增加和撤销本人非最后一个有效凭据。管理员可以撤销教师凭据；撤销
  教师最后一个有效凭据时必须原子撤销其会话并发放新邀请，且不得用该能力撤销最后管理员。
- 激活后的首次成功通行密钥认证在同一事务签发只展示一次、服务端只保存摘要的离线恢复码；
  管理员和 CLI 不接触原始码。紧急恢复必须同时满足有效恢复码和人工核验，恢复码本身不建立会话。
- 普通用户由有效管理员通过 Web/API 核验；最后管理员的 Web/API 审批必须返回
  `409 identity.last_admin_recovery_requires_cli` 且不得签发登记凭据。最后管理员只能由
  部署控制台 `init-admin recover-last-admin --recovery-request-id <uuid>` 交互匹配初始化时
  预登记的园所负责人和独立运维/安全责任人，在单一事务中写入两项不可变批准并签发 15 分钟
  单次浏览器登记凭据；CLI 不接收恢复码或 credential JSON，也不创建通行密钥。恢复完成后
  撤销旧密码、TOTP、通行密钥、全部会话、待处理邀请和旧恢复码；管理员必须重新配置备用
  登录。
- 微信小程序 MFA、短信/邮件验证码不进入首期认证关键路径；密码、TOTP 或恢复码均不得
  单独登录。

详细威胁、残余风险和生产门禁见 [`docs/security/threat-model.md`](../security/threat-model.md)。
...
- AI API Key 与必须可恢复验证的 TOTP 种子使用经认证加密后保存于 PostgreSQL；页面只显示
  AI Key 脱敏值和备用因素状态，不显示 TOTP 种子。两种信封按用途和 AAD 分离。
- 密钥读取封装为适配器，未来可替换为云密钥管理服务而不改变业务模块。

功能开发期间只定义密钥来源接口、权限受限的仓库外开发文件来源和测试适配器。生产适配器、
轮换操作和密钥带外恢复必须等 ADR-0009 复审后再实现；账号认证材料恢复按 ADR-0010/0011
执行，不与主加密密钥恢复混用。

### 13.2 安全边界

- 功能开发阶段不实施生产公网端口；最终入口虽已确定为受限公网且仅到 Web/BFF，实际防火墙、网关、域名和证书仍需在生产设计中验收。
...
首期全部功能完成前，不创建或验收生产 Caddyfile、生产 Compose 服务、公网 DNS、证书、端口映射、云主机目录或发布自动化。ADR-0010 冻结受限公网和 Web/BFF 唯一公网应用入口，不构成提前实施这些生产资源的授权。

本地开发和自动化测试可以使用 PostgreSQL、Redis 和临时导出目录，但相关启动方式只服务于可重复测试，不声称代表生产拓扑。

### 15.2 功能完成后的复审

威胁模型与访问网络方向已由 ADR-0010 冻结。功能验收后仍须据此选择并验证公网边界产品、生产运行单元、发布流程、密钥托管、持久化卷、备份介质、保留期、RPO/RTO 与恢复演练；不得把“受限公网”解释为沿用旧的 Caddy/Compose 假设。
...
- 认证安全测试覆盖 WebAuthn challenge、Origin/RP ID、UV、签名、重放、账号枚举、多凭据；
  密码策略/Argon2id、TOTP 时间窗口/原子重放、种子 AEAD、备用账号枚举/限流、管理员门禁、
  专用 `add_passkey` 证明；邀请泄露、双条件恢复和最后管理员恢复。
...
| 生产访问入口 | 受限公网；NiceGUI Web/BFF 是唯一公网应用入口 | 收敛扫描面，API、Worker 和数据服务保持私有；具体生产产品仍延后 |
| 首选登录 | WebAuthn 通行密钥，要求 RP/Origin 绑定和用户验证 | 提供钓鱼抗性 |
| 备用登录 | 密码与 TOTP 同意图双因素；普通业务可用，专用再验证只可新增通行密钥 | 降低设备切换成本，同时限制非钓鱼抗性路径的身份权限 |
| 会话传递 | 同源 HttpOnly Cookie，Web/BFF 服务端转发 | 浏览器脚本不接触令牌，API 统一授权 |
| 邀请与恢复 | 管理员单次邀请；离线恢复码加人工核验；最后管理员双人核验 | 泄露单一材料不能直接接管账号 |
...
2. 建立 PostgreSQL、Alembic、园所隔离 Repository 与一次性首位管理员初始化。
3. 实现 WebAuthn 登录、邀请与恢复、Cookie 会话、角色和教师—班级权限（M2）。
4. 实现首期必要设置（M3）。
5. 实现密码与 TOTP 双因素备用登录、认证保证级别和因素生命周期（M3A）。
6. 实现 AI Key 加密封装、提示词、AI 模型档案与测试运行。
7. 实现完全不依赖 AI 的教案创建、保存、快照、归档和恢复闭环。
8. 引入 Dramatiq、任务状态、恢复扫描、短轮询和栏目级 AI 生成。
9. 实现后台 Word 导出、私有文件下载和导出历史。
10. 完成功能、权限、数据隔离、审计和性能验收。
11. 全部功能完成后，依据已确认威胁模型、ADR-0010 和 ADR-0011 启动独立生产部署实现复审。
...
- 非身份业务 API 的后续端点、错误码、分页和契约 Schema。
- 生产环境会话规模、密钥轮换和监控告警细节；身份 ceremony、会话时长和撤销语义已由
  当前规格与 OpenAPI 冻结。
