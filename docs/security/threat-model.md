...
[ADR-0011](../ADR/ADR-0011-password-totp-backup-login.md)
...
- WebAuthn 通行密钥是首选且具备钓鱼抗性的登录方式；密码与 TOTP 两项共同成立可以建立
  独立备用会话，但任一单因素、短信/邮件验证码和微信小程序 MFA 均不是登录或恢复路径。
...
| 密码摘要、TOTP 种子信封和最后成功计数器 | 防止离线破解、密文替换、种子泄露和验证码重放 |
...
**场景**：攻击者伪造登录页、诱导用户输入密码与可转发 TOTP，撞库或窃取共享种子，或
窃取会话后执行管理员操作。
...
- 首选登录接受 WebAuthn 通行密钥；备用登录必须在一个不可观察中间结果的意图中同时验证
  密码和 TOTP，不提供任一单因素、短信/邮件验证码或微信小程序 MFA。
...
- 密码使用 Argon2id、独立随机盐、弱/泄露密码阻断和渐进重摘要；TOTP 使用每账号唯一高熵
  种子、6 位/30 秒、相邻一个时间步容差，并以原子持久计数器拒绝重放。
- TOTP 种子只展示一次，以 AES-256-GCM 加密并用园所、用户、凭据和版本作为 AAD；主密钥
  位于数据库和代码库之外。响应、日志、审计和测试快照不得包含密码、摘要、TOTP 或种子。
- 未知账号、未配置、密码错误、TOTP 错误、过期和重放返回相同公开失败；按可信来源、账号
  标识摘要和端点全局限流，不能借一条认证路径绕过或无关锁死另一条路径。
- 备用会话可使用角色允许的普通业务，但不能充当 WebAuthn 重新验证。最近五分钟再次验证
  密码与 TOTP 后只可新增通行密钥；删除凭据、因素维护、角色、邀请与恢复仍要求 WebAuthn。
- 用户可以绑定多个通行密钥；新增（除上述受限新设备路径外）、删除通行密钥和高风险管理员
  操作必须使用仍有效的通行密钥重新验证。
- 会话继续使用 `Secure`、`HttpOnly`、合适 `SameSite` 的 Cookie，并保留 CSRF、Origin 校验、短期 Access Token、Refresh Token family 轮换/撤销和账号停用后立即失权。
- 绑定新通行密钥、恢复账号、变更角色和撤销凭据均产生独立通知与审计事件。首期“通知”
  是登录后从现有审计事件投影的本人最近 20 条内建安全警报/列表，不新增外部投递或已读表。

**验收证据**：伪造 Origin/RP ID、challenge 重放、缺少 UV、未知凭据、备用登录账号枚举、
密码策略/摘要、TOTP 时间窗/重放、AEAD 标签失败、三层限流、Cookie 窃取后的会话撤销、
备用会话负向授权和敏感操作重新验证测试。

**残余风险**：通行密钥降低凭据钓鱼风险；密码与 TOTP 不具备钓鱼抗性，用户在伪造站点
同时输入两项时仍可能被实时转发。两种路径都不能消除被攻陷浏览器、恶意扩展、会话 Cookie
窃取或用户已登录后的操作诱导。
...
**场景**：用户更换、损坏或重置设备，当前设备没有可用通行密钥，或全部已绑定通行密钥
均不可用。
...
- 管理员必须预先配置密码与 TOTP 备用登录，教师可以选择配置并持续收到提示。两项同时
  正确时可在新设备建立普通业务会话；再次验证后只可新增通行密钥。
- 仍有通行密钥时，密码或 TOTP 遗失通过 WebAuthn 重新验证重设；TOTP 换设备启用新种子后
  立即作废旧种子、旧备用会话和未消费证明。
- 首次激活后生成至少 128 位随机熵的单次离线恢复码，只向用户展示一次；服务端只保存不可逆摘要并对校验限流。
- 用户仍有有效通行密钥时，可重新验证后新增凭据或轮换恢复码，不进入紧急恢复。
- 所有通行密钥均丢失且备用因素也不可用时，恢复必须同时满足有效离线恢复码和第 5.5 节
  人工核验；密码、TOTP 或恢复码任一项本身都不能重设备用因素或建立恢复会话。
- 恢复成功后撤销旧密码、TOTP、全部旧通行密钥、会话、未使用邀请和旧恢复码，绑定新
  通行密钥并签发新恢复码；管理员必须重新配置备用登录。

**验收证据**：多凭据生命周期、备用新设备流程、TOTP 换绑、恢复码只显示一次/摘要存储/
限流/单次使用、恢复后包含备用材料的全量撤销和通知测试。
...
- 备用会话必须显式标记认证方法；备用再验证只产生五分钟 `add_passkey` 专用证明，不自动
  提升当前会话，也不能用于其他敏感操作。
...
- 无单因素弱兜底：密码、TOTP、邀请链接、恢复码、短信/邮件、微信小程序或人工操作均不能
  单独登录；只有密码与 TOTP 共同成立的已确认备用路径可以建立普通业务会话。
- 认证保证级别：备用再验证只授权新增通行密钥；删除/重命名凭据、因素维护、角色变更、
  邀请、恢复和其他高风险身份操作需要新近 WebAuthn 用户验证。
- 全量撤销：账号停用、确认接管和紧急恢复必须撤销既有会话、通行密钥、备用材料和待处理邀请。
...
3. 密码与 TOTP 覆盖密码策略/Argon2id、时间窗口、计数器重放、种子 AEAD、账号枚举、三层
   限流、管理员强制、教师可选、普通业务会话和 `add_passkey` 专用证明负向矩阵。
4. 邀请覆盖签发、人工确认、过期、撤销、重放、泄露后抢先使用和日志无令牌。
5. 普通恢复与最后管理员恢复覆盖双条件、双人复核、冷静期、通知、审计，以及密码/TOTP 在内
   的全量撤销。
6. 个人设备失窃演练证明会话、通行密钥和备用材料可以远程处置，且下一请求立即失权。
7. 导出证明每次授权、短时鉴权下载和审计成立，并明确记录下载后外传是组织残余风险。
8. 扫描、限流、CSP、Cookie、CSRF、Origin、代理头、上传和依赖漏洞检查全部通过。
...
- [RFC 6238](https://www.rfc-editor.org/rfc/rfc6238.html)：TOTP 时间步、唯一种子和重放控制。
- [RFC 9106](https://www.rfc-editor.org/rfc/rfc9106.html)：Argon2id 参数与密码摘要基线。
