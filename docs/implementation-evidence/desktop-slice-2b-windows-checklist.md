...
状态：**等待 Ubuntu 修复验收后重测**。2026-08-12 已由用户手工确认源码启动与
`0001 -> 0002` 迁移保护通过；之后的 Windows Agent 行为测试暴露 Issue #22。修复后的顺序固定为
先完成 Ubuntu 功能测试，再重跑 Windows。两种平台证据互不替代。
...
开始新的 Windows 手工测试前，先确认远端 `dev` 的 Issue #22 精确提交已在 Ubuntu 通过：锁定同步、
Ruff、Pyright、完整 Pytest、隔离模块入口和 wheel/sdist 构建。Windows 使用同一提交，不沿用旧构建。

本轮新增的手工观察范围：

- [ ] 设置页可编辑并重启保留园所、教师、当前班级、年龄段、室内区域和户外区域。
- [ ] 文本模型与视觉模型分别保存；Key 不回显且 SQLite 中不存在明文。视觉模型本期不发起请求。
- [ ] 集体活动页可粘贴完整原稿，保存和重启后原稿仍在，原稿不替代结构化栏目。
- [ ] 输入集体活动原稿后可点击“AI 拆分原稿”；结果先预览，只有教师明确采用后
  才更新结构化栏目，且原稿仍保留。
- [ ] 内置提示词能返回当前关闭 JSON 结构，但教师页面以中文字段和列表显示，不显示
  JSON 原文或 Markdown 包裹内容。
- [ ] 生成室内区域游戏、下午户外游戏和晨间活动时，结果只使用当前班级已保存的室内/
  户外区域，不虚构未配置场地。
- [ ] 晨间活动结果是户外体育活动：体能大循环、集体体育游戏和自主体育游戏，不是
  室内教学、谈话或静态活动。
- [ ] 慢速 AI 请求在合理等待内成功；最终失败保留原正文并给出可理解提示。
- [ ] Agent 至少完成一次 READ 后 DRAFT；Provider 使用界面当前的 plan/class/date 标识，不再
  出现“Agent Tool 请求未被允许”，也不因缺失 assistant tool-call 消息而拒绝续接。
- [ ] 既有 `0002_desktop_ai` 或 `0003_desktop_ai_profiles` 活动库在每次实际迁移前生成对应
  保护副本，活动库最终到达 `0004_desktop_group_activity_ai`，既有数据保留且重启后不重复创建
  迁移保护副本。
