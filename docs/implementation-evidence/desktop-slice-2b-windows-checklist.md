...
- [ ] 内置提示词能返回当前关闭 JSON 结构，页面不出现 Markdown 包裹内容。
- [ ] 慢速 AI 请求在合理等待内成功；最终失败保留原正文并给出可理解提示。
- [ ] Agent 至少完成一次 READ 后 DRAFT；Provider 不再因缺失 assistant tool-call 消息而拒绝续接。
- [ ] 既有 `0002_desktop_ai` 活动库升级前生成 `0002` 保护副本，活动库到达
  `0003_desktop_ai_profiles`，既有数据保留且重启后不重复创建迁移保护副本。
