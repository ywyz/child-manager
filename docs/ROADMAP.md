...
- 生产部署设计。ADR-0010 已冻结受限公网、Web/BFF 唯一公网应用入口、WebAuthn 与账号恢复
  边界；ADR-0011 增加密码与 TOTP 双因素备用登录。根据 ADR-0009，反向代理产品、生产
  拓扑、密钥托管、备份恢复和上线流程仍在全部功能完成后另行复审。
...
现为 `complete`，US4 T087–T110 已完成，继续遵守 `0006_lesson_plans →
...
US5 T111–T126 已完成，T126 验收为设置页 49 passed、US5 专项 56 passed、完整 pytest
...
- 离线恢复码加人工核验；普通账号由有效管理员通过 Web/API 核验。最后管理员的 Web/API
  恢复审批返回 `409 identity.last_admin_recovery_requires_cli`，只能由部署控制台
  `init-admin recover-last-admin --recovery-request-id <uuid>` 交互匹配预登记园所负责人和
  独立运维/安全责任人，原子记录两项批准并签发 15 分钟单次浏览器登记凭据。
...
入口条件：M3 `complete`。本阶段使用独立 Issue，不扩大 M3 #7 的 T036–T045。

### 交付范围

- `0005_password_totp_backup_login.py` 以 `0004_settings` 为父迁移，建立独立备用凭据与
  enrollment 表，并扩展会话认证方法和备用因素版本。
- 管理员强制、教师可选地建立密码与 TOTP；密码使用 Argon2id，TOTP 种子使用
  AES-256-GCM 与数据库外主密钥。
- 密码与 TOTP 同时验证的备用登录建立普通业务会话；失败不可枚举且实施三层限流。
- 备用会话再次验证两项因素后只可新增通行密钥，不能执行其他高风险身份操作。
- WebAuthn 保护备用因素建立、轮换和关闭；紧急恢复继续采用离线恢复码加人工核验并撤销
  全部旧认证材料。
...
- API 校验权限、当前教案版本和前五个关键栏目缺失的软确认，创建后台导出任务；一日活动反思为空不触发确认，但模板仍保留反思三行位置。
...
- 已确认的 [`docs/security/threat-model.md`](security/threat-model.md)、ADR-0010 及
  ADR-0011。
...
- API、Worker、Redis、PostgreSQL 和运维端口不得单独暴露公网。
- WebAuthn 通行密钥是首选且具备钓鱼抗性的登录方式；密码与 TOTP 两项共同成立提供备用
  登录，任一单因素均不得登录；账号恢复采用离线恢复码加人工核验。
...
- [x] 威胁模型、访问网络和认证保证级别由 ADR-0010/ADR-0011 冻结。
