...
- [ ] 经明确授权后，`codex` 和 `trae` 从同一最终 `main` 提交创建。
- [ ] 两个分支均记录相同基线提交，不带未审阅业务代码。
...
| 里程碑 | 状态 | 当前证据 | 下一个解锁动作 |
| --- | --- | --- | --- |
| M0 共享基线 | `complete` | M0-G1～M0-G8 已关闭；历史隐私清理完成，脱敏模板和最终 docs-only `main` 基线已通过清理后专项验证与远端重克隆复核 | 保持 `main` 为共享文档基线；实现分支操作另行授权 |
| M1 | `ready` | M0 已完成；当前仍无 `pyproject.toml`、`uv.lock`、业务代码、迁移或自动化测试 | 经授权执行 T003，从同一最终 `main` 提交创建 `codex` 与 `trae`，再分别开始工程骨架 |
