...
def test_postgres_worker_rebuilds_frozen_context_and_finishes_the_public_job(
    ...
    with psycopg.connect(native_url) as connection:
        audit = connection.execute(
            """SELECT outcome,resource_id FROM audit_events
            WHERE kindergarten_id=%s AND event_code='prompt.test_attempted'""",
            (_actor.kindergarten_id,),
        ).fetchone()
    assert audit == ("success", UUID(accepted.json()["related_resource_id"]))
...
def test_recovery_reservation_stays_pending_until_redis_dispatch_succeeds(
    ...
    accepted = client.post(
        ...
        headers={**csrf_headers(client), "Idempotency-Key": "recovery-pending"},
    )
    job_id = UUID(accepted.json()["job"]["id"])
    native_url = isolated_database_url.replace("postgresql+psycopg://", "postgresql://", 1)
    with psycopg.connect(native_url) as connection:
        connection.execute(
            """UPDATE background_jobs SET updated_at=now()-interval '16 seconds'
            WHERE kindergarten_id=%s AND id=%s""",
            (actor.kindergarten_id, job_id),
        )
    ...
    assert client.get(f"/api/v1/jobs/{job_id}").json()["status"] == "pending_dispatch"
