...
def test_ai_generation_results_revision_is_declared_as_head() -> None:
    heads = ScriptDirectory.from_config(Config("alembic.ini")).get_heads()

    assert heads == ["0008_ai_generation_results"]
...
def test_empty_database_can_upgrade_to_current_head(isolated_database_url: str) -> None:
    ...
    assert "0008_ai_generation_results" in current.stdout
